...
def add_number():
    """
    Tambah nomor dari Test Numbers ke My Numbers.

    CONFIRMED dari source JS iVAS:
      function AddNumbers(id) {
          $.ajax({
              url: ".../portal/numbers/termination/number/add",
              type: "post",
              dataType: "json",
              data: { _token: "...", id: id }   ← hanya 'id' saja!
          })
      }

    Params:
      termination_id : ID dari Test Numbers (field number_id di /numbers/test-list)
      number         : (alternatif) nomor telepon → auto-resolve ke termination_id
      account        : (opsional) email akun, default: akun pertama
    Contoh:
      /numbers/add?termination_id=82774
      /numbers/add?number=923008264692
      /numbers/add?number=923008264692&account=email@x.com
    """
    ...
    if not termination_id and not number:
        return jsonify({
            "error":   "Parameter termination_id atau number wajib",
            "contoh_1": "/numbers/add?termination_id=82774",
            "contoh_2": "/numbers/add?number=923008264692",
            "tip":     "Cek /numbers/test-list untuk lihat termination_id (field number_id)",
        }), 400
    ...
    # ── Resolve termination_id dari nomor ────────────────────────────────────
    ...
    if not termination_id and number:
        ...
        if not termination_id:
            return jsonify({
                "status": "error",
                "error":  f"Nomor {number} tidak ditemukan di Test Numbers akun {email}",
                "tip":    "Cek /numbers/test-list untuk lihat nomor yang tersedia",
            }), 404

    # ── POST add — hanya kirim 'id' sesuai JS iVAS ───────────────────────────
    try:
        resp, _ = do_request(
            account, "POST",
        ...
        logger.info(f"[ADD] {email}: id={termination_id} number={resolved_number} success={success} msg={message}")
...
# ════════════════════════════════════════════════════════
# /numbers/add-by-range — add SEMUA nomor dari 1 range
# ════════════════════════════════════════════════════════
#
# Flow:
#   1. Fetch Test Numbers, filter by range_name
#   2. Ambil semua termination_id (field id / DT_RowId)
#   3. Loop POST /portal/numbers/termination/number/add {id: each_id}
#
# ════════════════════════════════════════════════════════
...
def add_numbers_by_range():
    """
    Tambah SEMUA nomor dalam 1 range dari Test Numbers ke My Numbers.

    Flow otomatis:
      1. Fetch Test Numbers, filter by range_name
      2. Loop add satu per satu (POST /portal/numbers/termination/number/add)

    Params:
      range_name : nama range, misal "PAKISTAN 34" — WAJIB
      account    : (opsional) email akun, default akun pertama
      limit      : (opsional) max nomor yang di-add, default semua (500)
      dry_run    : (opsional) "1" → hanya list nomor, tidak add. Default "0"

    Contoh:
      /numbers/add-by-range?range_name=PAKISTAN 34
      /numbers/add-by-range?range_name=AFGHANISTAN 1000
      /numbers/add-by-range?range_name=PAKISTAN 34&account=email@x.com
      /numbers/add-by-range?range_name=PAKISTAN 34&dry_run=1   ← preview dulu
      /numbers/add-by-range?range_name=PAKISTAN 34&limit=10    ← max 10 nomor
    """
    ...
    if not range_name:
        return jsonify({
            ...
            "contoh_1": "/numbers/add-by-range?range_name=PAKISTAN 34",
            "contoh_2": "/numbers/add-by-range?range_name=AFGHANISTAN 1000",
            "tip":      "Cek /numbers/test-list untuk lihat range yang tersedia",
        }), 400
    ...
    # ── Step 1: Fetch Test Numbers, filter by range_name ─────────────────────
    logger.info(f"[ADD-RANGE] {email}: fetch range='{range_name}' limit={limit}")
    ...
    for row in rows:
        ...
        if rng.lower() == range_name.lower():
            tid = str(row.get("id","") or row.get("DT_RowId","")).strip()
            num = re.sub(r"<[^>]+>","",str(row.get("test_number",""))).strip()
            if tid and tid.isdigit():
                matched.append({"termination_id": tid, "number": num, "range": rng})

    logger.info(f"[ADD-RANGE] {email}: total_iVAS={total} matched={len(matched)}")

    if not matched:
        return jsonify({
            "status":     "error",
            "error":      f"Tidak ada nomor ditemukan untuk range '{range_name}' di akun {email}",
            "total_iVAS": total,
            "tip":        "Cek /numbers/test-list untuk lihat range yang tersedia",
        }), 404

    # ── Dry run: hanya return list ────────────────────────────────────────────
    if dry_run:
        return jsonify({
            "status":      "dry_run",
            "range_name":  range_name,
            ...
            "total_iVAS":  total,
            "numbers":     matched,
            "tip":         "Hapus &dry_run=1 untuk eksekusi add semua nomor ini",
        })

    # ── Step 2: Add satu per satu ─────────────────────────────────────────────
    ...
    for item in matched:
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"[ADD-RANGE] {email}: ✅ id={tid} num={num} msg={message}")
            else:
                # "Too Many Attempts" → rate limited, stop
                if "too many" in message.lower():
                    skipped.append(entry)
                    logger.warning(f"[ADD-RANGE] {email}: rate limit hit at id={tid}, stopping")
                    break
                # "maximum number" → akun sudah penuh
                elif "maximum" in message.lower():
                    skipped.append(entry)
                    logger.warning(f"[ADD-RANGE] {email}: akun penuh di id={tid}, stopping")
                    break
                else:
                    errors.append(entry)
                    logger.warning(f"[ADD-RANGE] {email}: ❌ id={tid} msg={message}")

            # Delay kecil untuk hindari rate limit
            _time.sleep(0.3)

        except Exception as e:
            errors.append({"termination_id": tid, "number": num, "success": False, "error": str(e)})
            logger.error(f"[ADD-RANGE] {email}: error id={tid}: {e}")

    total_skipped = len(matched) - len(results) - len(errors) - len(skipped)
    ...
    return jsonify({
        ...
        "failed":          len(errors),
        "skipped":         len(skipped),
        "not_processed":   max(total_skipped, 0),
        "results":         results,
        "errors":          errors,
        "skipped_details": skipped,
        "tip":             "Jika skipped > 0 karena rate limit, coba lagi beberapa menit kemudian",
