...
def debug_cf_login():
    """
    Dump RAW response dari setiap step login iVAS.
    Untuk diagnosa Cloudflare / Turnstile issues.
    Usage: /debug/cf-login?email=kicenofficial@gmail.com
    """
    import urllib.parse
    ...
    SEP = "=" * 60
    out = []
    out.append("[DEBUG CF-LOGIN] Raw iVAS Login Diagnostic")
    ...
    try:
        ...
        out.append("STEP 1: GET https://www.ivasms.com/login")
        out.append("-" * 40)
        r1 = sc.get(LOGIN_URL, timeout=20, allow_redirects=True)
        h1 = decode_response(r1)

        out.append(f"Status       : {r1.status_code}")
        out.append(f"Final URL    : {r1.url}")
        out.append(f"Size         : {len(h1)} chars")
        out.append(f"Content-Type : {r1.headers.get('Content-Type', '')}")
        out.append("")

        # CF Detection
        h1_low = h1.lower()
        cf_signs = {
            "Turnstile widget": "cf-turnstile" in h1_low,
            "Just a moment":    "just a moment" in h1_low,
            "CF challenge":     "challenges.cloudflare" in h1_low,
            "CF ray":           "cf-ray" in str(r1.headers).lower(),
            "503/403 CF":       r1.status_code in (403, 503),
            "Login form ada":   'name="_token"' in h1 or 'csrf-token' in h1,
        }
        out.append("[ CF Detection ]")
        for k, v in cf_signs.items():
            out.append(f"  {k}: {'⚠️ YES' if v else '✅ NO'}")
        out.append("")

        # Cookies dari response
        out.append("[ Cookies setelah GET ]")
        for ck in sc.cookies:
            out.append(f"  {ck.name} = {str(ck.value)[:60]}...")
        out.append("")

        # Response headers penting
        out.append("[ Response Headers ]")
        for k in ("Server", "cf-ray", "cf-cache-status", "x-frame-options",
                  "set-cookie", "location", "content-security-policy"):
            v = r1.headers.get(k, "")
            if v:
        ...
        # HTML snippet awal
        out.append("[ HTML first 800 chars ]")
        out.append(h1[:800])
        out.append("...")
        out.append("")
        ...
        meta_tok = soup1.find("meta", {"name": "csrf-token"})
        tok = (tok_el["value"] if tok_el else None) or (meta_tok["content"] if meta_tok else None)

        out.append(f"[ _token found ] {'YES: ' + tok[:30] + '...' if tok else 'NO ← problem!'}")
        out.append(SEP)

        if not tok:
            out.append("STOP: Tidak ada _token — CF blocking GET login page")
            out.append("")
            out.append("SOLUSI:")
            out.append("  iVAS sekarang pakai Cloudflare Turnstile.")
            out.append("  cloudscraper/requests tidak bisa bypass ini secara otomatis.")
            out.append("")
            out.append("  Option 1 — CF_CLEARANCE cookie:")
            out.append("    1. Buka https://www.ivasms.com/login di browser")
            out.append("    2. Solve Turnstile checkbox")
            out.append("    3. DevTools → Application → Cookies → copy 'cf_clearance'")
            out.append("    4. Set env var CF_CLEARANCE=<nilai>")
            out.append("    5. Set env var CF_USER_AGENT=<UA dari browser yang sama>")
            out.append("")
            out.append("  Option 2 — Cari API/endpoint alternatif iVAS yang tidak kena CF")
            return Response("\n".join(out), mimetype="text/plain; charset=utf-8")

        # ── STEP 2: POST login ──
        out.append("STEP 2: POST login dengan credentials")
        out.append("-" * 40)
        r2 = sc.post(
            LOGIN_URL,
            data={"email": target["email"], "password": target["password"], "_token": tok},
            headers={
        ...
        out.append(f"Final URL : {r2.url}")
        out.append(f"Size      : {len(h2)} chars")
        out.append("")

        h2_low = h2.lower()
        post_result = {
            "Redirect ke portal": "/portal" in r2.url,
            "Masih di /login":    "/login" in r2.url,
            "CF block":           "just a moment" in h2_low or r2.status_code in (403,503),
            "Invalid credentials":"invalid" in h2_low or "credentials" in h2_low,
            "Turnstile error":    "turnstile" in h2_low,
        }
        ...
        out.append("[ Cookies setelah POST ]")
        for ck in sc.cookies:
            out.append(f"  {ck.name} = {str(ck.value)[:80]}")
        out.append("")

        out.append("[ HTML first 600 chars ]")
        out.append(h2[:600])
        out.append("...")
        out.append(SEP)

        # ── STEP 3: Kalau berhasil, coba portal ──
        if "/portal" in r2.url or "/login" not in r2.url:
            out.append("STEP 3: Login berhasil! GET portal...")
            r3 = sc.get("https://www.ivasms.com/portal/sms/received", timeout=15)
            h3 = decode_response(r3)
            out.append(f"Portal status: {r3.status_code}  Size: {len(h3)}")
            out.append(f"Portal URL  : {r3.url}")
            out.append("[ Portal HTML 400 chars ]")
            out.append(h3[:400])
        else:
            out.append("STEP 3: Login gagal — tidak bisa lanjut ke portal")

        out.append(SEP)
        out.append("Usage: /debug/cf-login?email=kicenofficial@gmail.com")
