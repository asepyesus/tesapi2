...
# Key: email → socketio.Client instance
_ws_clients: dict = {}
...
def _ws_connect_account(account: dict):
    """
...
def live_my_sms():
    """
    Live SMS masuk ke nomor akun sendiri (Client System).
    Scrape dari /portal/live/my_sms — tabel Live SMS.

    Query params:
      account=email   → filter akun tertentu (default: akun pertama)
      limit=N         → max hasil (default: 50)
      raw=1           → tampilkan raw HTML (debug)

    Response JSON:
      {status, account, total, sms: [{range, number, sid, paid, limit, message, received_at}]}
    """
    ...
    target_acc = None
    for acc in ACCOUNTS:
        if not acc_filter or acc["email"] == acc_filter:
            target_acc = acc
            break
    ...
    # Parse tabel Live SMS
    soup  = BeautifulSoup(html, "html.parser")
    items = []

    # Cari tabel dengan header Live SMS / SID / Paid / Limit / Message content
    tables = soup.find_all("table")
    target_table = None
    for t in tables:
        headers = [th.get_text(strip=True).lower() for th in t.find_all("th")]
        if any(h in headers for h in ("sid", "paid", "limit", "message content")):
            target_table = t
            break
    ...
    if target_table:
        ...
        # Fallback: cari semua baris yang ada konten SMS
        for row in soup.find_all("tr"):
            cols = [td.get_text(strip=True) for td in row.find_all("td")]
            if len(cols) >= 3 and any(cols):
                items.append({
                    "range":      cols[0] if len(cols) > 0 else "",
                    "number":     cols[1] if len(cols) > 1 else "",
                    "sid":        cols[2] if len(cols) > 2 else "",
...
def live_public_sms():
    """
    Public SMS History — scrape dari /portal/sms/test/sms.
    Ini tabel history semua SMS test (gambar 2: SMS Test History).
    ...
    Query params:
      ...
      raw=1           → tampilkan raw HTML

    Response JSON:
      {status, total, sms: [{range_name, number, sid, message, received_at}]}
    """
    PUBLIC_SMS_URL = f"{BASE_URL}/portal/sms/test/sms"
    acc_filter = request.args.get("account", "").strip()
    limit      = int(request.args.get("limit", 50))
    sid_filter = request.args.get("sid",   "").strip().lower()
    ...
    scraper = sess["scraper"]
    try:
        r    = scraper.get(PUBLIC_SMS_URL, timeout=15,
                           headers={"Referer": f"{BASE_URL}/portal/live/test_sms"})
        html = decode_response(r)
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
    ...
    # Cari tabel history — header: Range Name, Test Number, SID, Message content, Received time
    tables = soup.find_all("table")
    target_table = None
    for t in tables:
        headers_text = t.get_text(separator="|").lower()
        if "range" in headers_text and ("sid" in headers_text or "message" in headers_text):
            target_table = t
            break
    ...
    if target_table:
        ...
        for row in rows:
            cols = [td.get_text(strip=True) for td in row.find_all("td")]
            if len(cols) >= 3:
                range_name   = cols[0] if len(cols) > 0 else ""
                test_number  = cols[1] if len(cols) > 1 else ""
                sid          = cols[2] if len(cols) > 2 else ""
                message      = cols[3] if len(cols) > 3 else ""
                received_at  = cols[4] if len(cols) > 4 else ""

                # Apply filters
                if sid_filter and sid_filter not in sid.lower() and sid_filter not in message.lower():
                    continue
                if rng_filter and rng_filter not in range_name.lower():
                    continue
