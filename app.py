...
# ── Cache public SMS (send_message_max_Limit_217253) ────
...
def _scrape_jwt_token(scraper, page_url: str) -> str | None:
    """
    Ambil JWT/session token untuk Socket.IO iVAS.

    iVAS pakai Cloudflare Rocket Loader yang obfuscate semua script —
    jadi console.log JWT TIDAK ada di raw HTML.

    Strategi yang benar:
      1. Intercept Socket.IO polling handshake — server kirim token di
         response polling pertama (EIO=4&transport=polling)
      2. Cari token di response JSON Socket.IO (/socket.io/?EIO=4...)
      3. Fallback: ambil dari cookie session (laravel_session) yang
         bisa dipakai sebagai auth

    Return: token string atau None
    """
    ...
    try:
        ...
        # ── Pattern 1: eyJ... JWT langsung di HTML (kalau Rocket Loader off) ──
        all_jwts = re.findall(
        ...
        if all_jwts:
            tok = all_jwts[0]
            logger.info(f"[WS-JWT] ✅ JWT dari HTML eyJ pattern: {tok[:30]}...")
            return tok

        # ── Pattern 2: Intercept Socket.IO polling handshake ──
        # iVAS Socket.IO di port 2087, endpoint /socket.io/
        # GET ?EIO=4&transport=polling → server reply JSON dengan sid
        # Kita ambil cookies session dari scraper untuk auth
        try:
            ...
            # Hit polling endpoint — ini yang iVAS browser lakukan pertama
            ...
            poll_body = pr.text
            logger.info(f"[WS-JWT] Polling response HTTP {pr.status_code}: {poll_body[:120]}")

            # Response biasanya: 0{"sid":"xxx","upgrades":["websocket"],...}
            # Atau bisa ada token di JSON
            tok_m = re.search(
                r'\b(eyJ[A-Za-z0-9_\-]{10,}\.[A-Za-z0-9_\-]{10,}\.[A-Za-z0-9_\-]{10,})\b',
                poll_body
            )
            if tok_m:
                tok = tok_m.group(1)
                logger.info(f"[WS-JWT] ✅ JWT dari polling response: {tok[:30]}...")
                return tok

            # Kalau polling berhasil (sid ada) → pakai laravel_session sebagai token
            if '"sid"' in poll_body and pr.status_code == 200:
                # Extract sid untuk info
                sid_m = re.search(r'"sid"\s*:\s*"([^"]+)"', poll_body)
                sid = sid_m.group(1) if sid_m else ""
                logger.info(f"[WS-JWT] Polling OK (sid={sid[:20]}) — pakai laravel_session")

        except Exception as e:
            logger.debug(f"[WS-JWT] Polling attempt error: {e}")

        # ── Pattern 3: Gunakan laravel_session cookie sebagai auth token ──
        # Beberapa Socket.IO server iVAS menerima session cookie langsung
        laravel_session = scraper.cookies.get("laravel_session", "")
        if laravel_session:
            logger.info(f"[WS-JWT] ✅ Pakai laravel_session cookie: {laravel_session[:30]}...")
            return laravel_session

        # ── Pattern 4: Cari token di semua response cookies ──
        for name, val in scraper.cookies.items():
            if len(val) > 20 and name.lower() in ("token", "auth_token", "jwt", "access_token"):
                logger.info(f"[WS-JWT] ✅ Token dari cookie '{name}': {val[:30]}...")
                return val

        # ── Pattern 5: Cari di JS files yang di-load halaman ──
        soup = BeautifulSoup(html, "html.parser")
        for script in soup.find_all("script", src=True):
            src = script.get("src", "")
            if "ivasms.com" in src and any(k in src for k in ("app", "main", "bundle", "portal")):
                try:
                    full_src = src if src.startswith("http") else BASE_URL + src
                    js_r = scraper.get(full_src, timeout=10)
                    js_body = js_r.text
                    tok_js = re.findall(
                        r'\b(eyJ[A-Za-z0-9_\-]{10,}\.[A-Za-z0-9_\-]{10,}\.[A-Za-z0-9_\-]{10,})\b',
                        js_body
                    ...
                    if tok_js:
                        logger.info(f"[WS-JWT] ✅ JWT dari JS file {src[-40:]}: {tok_js[0][:30]}...")
                        return tok_js[0]
                except Exception:
                    pass

        logger.warning(f"[WS-JWT] ❌ Semua strategi gagal di {page_url}")
...
def _ws_add_public(email: str, sms_data: dict):
    """Tambah SMS ke cache public feed (send_message_max_Limit_217253)."""
...
def _build_ws_client(email: str, jwt_token: str) -> "_sio_module.Client | None":
    ...
    try:
        ...
        def on_send_message_test(data):
            ...
            Payload dari iVAS:
              data.cli            → nomor pengirim (dengan +)
              data.test_number    → nomor test iVAS
              data.message        → isi SMS / OTP
              data.termination_id → ID terminasi
            """
            try:
                cli     = str(data.get("cli", "")).replace("+", "")
                number  = str(data.get("test_number", ""))
                ...
                logger.info(
                    f"[WS] 📨 SMS [{email[:20]}] "
                    f"from={cli} num={number} msg={message[:40]}"
                )
            except Exception as e:
        ...
        # ── Handler send_message_max_Limit_217253 — Public SMS history ──
...
def _build_livesms_client(email: str, jwt_token: str) -> "_sio_module.Client | None":
    """
    Buat Socket.IO client untuk namespace /livesms — My SMS (nomor akun sendiri).
    iVAS pakai: window.liveSMSSocket = io('https://ivasms.com:2087/livesms', {...})
    """
    try:
        sio = _sio_module.Client(
            reconnection=True,
        ...
        @sio.event
        def connect():
            logger.info(f"[WS-LIVE] ✅ {email} — Connected ke /livesms namespace")
        ...
        @sio.on("send_message_live")
        def on_live(data):
            """SMS masuk ke nomor akun sendiri."""
            try:
                cli     = str(data.get("cli", "")).replace("+", "")
                number  = str(data.get("number", data.get("live_number", "")))
                message = str(data.get("message", ""))
                term_id = str(data.get("termination_id", data.get("range", "")))
                ...
                logger.info(f"[WS-LIVE] 📨 MY SMS [{email[:20]}] from={cli} num={number} msg={message[:40]}")
            except Exception as e:
                logger.error(f"[WS-LIVE] on_send_message_live error {email}: {e}")

        @sio.on("send_message_max_Limit_217253")
        def on_limit(data):
            """SMS yang masuk tapi sudah melebihi limit — tetap simpan."""
            try:
                message = str(data.get("message", ""))
                number  = str(data.get("number", data.get("live_number", "")))
                cli     = str(data.get("cli", "")).replace("+", "")
                entry = {
                    "range":        str(data.get("termination_id", "")),
                    "phone_number": cli or number,
                    "number":       number,
                    "otp_message":  message,
                    "message":      message,
                    "sid":          str(data.get("sid", "")),
                    "paid":         "limit_exceeded",
                    "source":       "websocket_limit",
                    "account":      email,
                    "received_at":  datetime.now().isoformat(),
                    "raw":          data,
                }
                _ws_add_live(email, entry)
                logger.info(f"[WS-LIVE] ⚠️ LIMIT SMS [{email[:20]}] msg={message[:40]}")
            except Exception as e:
        ...
        def on_any_live(event, data):
            if event not in ("connect", "disconnect", "connect_error",
                             "send_message_live", "send_message_max_Limit_217253"):
                logger.debug(f"[WS-LIVE] Unknown event '{event}' [{email[:20]}]: {str(data)[:100]}")
...
def _ws_connect_livesms(account: dict):
    ...
    while _ws_enabled:
        try:
            ...
            sio = _build_livesms_client(email, jwt_token or "")
            if not sio:
                time.sleep(reconnect_delay)
                continue
            ...
            logger.info(f"[WS-LIVE] {email} → Connecting ke {WS_URL}/livesms ...")
...
def live_my_sms():
    """
    Live SMS masuk ke nomor akun sendiri (My Numbers).
    Data real-time dari WebSocket /livesms namespace.
    Fallback: scrape HTML tabel /portal/live/my_sms.

    Query params:
      account=email   → filter akun tertentu
      limit=N         → max hasil (default: 50)
      source=ws       → hanya dari WebSocket (skip scrape)
      raw=1           → raw HTML halaman (debug)

    Response JSON:
      {status, source, total, sms: [{range, number, sid, paid, message, account, received_at}]}
    """
    MY_SMS_URL = f"{BASE_URL}/portal/live/my_sms"
    acc_filter = request.args.get("account", "").strip() or None
    limit      = int(request.args.get("limit", 50))
    force_ws   = request.args.get("source", "").strip() == "ws"
    show_raw   = request.args.get("raw", "0") == "1"

    # ── Coba dari WebSocket cache dulu (real-time) ──
    ...
    if ws_items:
        ...
        for s in ws_items:
            clean.append({
                ...
                "source":      s.get("source", "websocket_livesms"),
            })
        return jsonify({
            "status":  "success",
            "source":  "websocket_livesms",
    ...
    if force_ws:
        return jsonify({"status": "success", "source": "websocket_livesms",
                        "total": 0, "sms": [], "note": "Cache kosong, tunggu SMS masuk"})

    # ── Fallback: scrape HTML ──
    target_acc = next((a for a in ACCOUNTS if not acc_filter or a["email"] == acc_filter), None)
    if not target_acc:
    ...
    scraper = sess["scraper"]
    try:
        r    = scraper.get(MY_SMS_URL, timeout=15)
        html = decode_response(r)
    except Exception as e:
    ...
    for t in tables:
        ...
        if "sid" in headers or "paid" in headers or "message" in headers:
            for row in t.find_all("tr")[1:limit+1]:
                cols = [td.get_text(strip=True) for td in row.find_all("td")]
                if len(cols) >= 3 and any(c for c in cols):
                    items.append({
                        "range":       cols[0] if len(cols) > 0 else "",
                        "number":      cols[1] if len(cols) > 1 else "",
                        "sid":         cols[2] if len(cols) > 2 else "",
                        "paid":        cols[3] if len(cols) > 3 else "",
                        "message":     cols[4] if len(cols) > 4 else "",
                        "account":     target_acc["email"],
                        "received_at": datetime.now().isoformat(),
                        "source":      "scrape_my_sms_html",
                    })
            break

    # ── Fallback XHR: scrape /portal/sms/received/getsms ──
    # Kalau tabel HTML kosong (Vercel serverless, JS-rendered table)
    if not items:
        try:
            sess_recv = get_session(target_acc)
            if sess_recv and sess_recv.get("ok"):
                recv_csrf = sess_recv.get("recv_csrf", sess_recv.get("csrf", ""))
                recv_data = {
                    "_token": recv_csrf,
                ...
                resp_recv, _ = do_request(
                    target_acc, "POST",
                    f"{BASE_URL}/portal/sms/received/getsms",
                    data=recv_data, headers=hdrs_recv
                )
                if resp_recv and resp_recv.status_code == 200:
                    try:
                        rdata = resp_recv.json()
                        # Response bisa list langsung atau dict dgn key "data"
                        rrows = rdata if isinstance(rdata, list) else rdata.get("data", [])
                        for rrow in rrows[:limit]:
                            rng = html_lib.unescape(re.sub(r'<[^>]+>', '', str(rrow.get("range", rrow.get("range_name", ""))))).strip()
                            num = html_lib.unescape(re.sub(r'<[^>]+>', '', str(rrow.get("number", rrow.get("test_number", ""))))).strip()
                            sid = html_lib.unescape(re.sub(r'<[^>]+>', '', str(rrow.get("sid", rrow.get("originator", ""))))).strip()
                            msg = html_lib.unescape(re.sub(r'<[^>]+>', '', str(rrow.get("message", rrow.get("messagedata", ""))))).strip()
                            rcv = str(rrow.get("received_at", rrow.get("senttime", datetime.now().isoformat())))
                            if any([rng, num, sid, msg]):
                                items.append({
                                    "range":       rng,
                                    "number":      num,
                                    "sid":         sid,
                                    ...
                                    "account":     target_acc["email"],
                                    "received_at": rcv,
                                    "source":      "scrape_xhr_received",
                                })
                    except Exception as ep:
                        logger.warning(f"[MY-SMS] parse getsms error: {ep}")
        except Exception as ef:
            logger.warning(f"[MY-SMS] fallback XHR error: {ef}")

    return jsonify({
        "status":  "success",
        "source":  items[0].get("source", "scrape_my_sms") if items else "scrape_my_sms",
        "account": target_acc["email"],
        "total":   len(items),
        "sms":     items,
        "note":    "Data kosong — nomor My SMS mungkin belum ada SMS masuk hari ini" if not items else "",
    })
...
def live_test_sms():
    ...
    # ── Fallback: WebSocket kosong di Vercel → scrape XHR langsung ──
    if not items:
        try:
            target_acc = next((a for a in ACCOUNTS if not acc_filter or a["email"] == acc_filter), ACCOUNTS[0])
            sess = get_session(target_acc)
            if sess and sess.get("ok"):
                scraper    = sess["scraper"]
                ...
                qs = (
                    "draw=1"
                    "&columns[0][data]=0&columns[0][name]=0"
                ...
                resp, _ = do_request(target_acc, "GET", f"{PUBLIC_URL}?{qs}", headers=hdrs)
                if resp and resp.status_code == 200:
                    jdata = resp.json()
                    rows  = jdata.get("data", [])

                    def _cf(raw):
                        s = html_lib.unescape(str(raw))
                    ...
                    def _clean_msg_field(raw: str) -> str:
                        # Decode double-encoded HTML entities, strip HTML tags, fix unicode
                        s = str(raw)
                        # Decode \uXXXX literal string (bukan unicode char, tapi backslash-u)
                        try:
                            s = s.encode('utf-8').decode('unicode_escape', errors='replace').encode('latin-1').decode('utf-8', errors='replace')
                        except Exception:
                            pass
                    ...
                    for row in rows:
                        if isinstance(row, dict):
                            ...
                            msg = _clean_msg_field(str(row.get("messagedata", "")))
                            rcv = str(row.get("senttime", ""))
                            rng = _cf(str(row.get("range", "")))
                        elif isinstance(row, list) and len(row) >= 4:
                            raw_sid = str(row[2])
                            raw_num = str(row[1])
                            msg     = _clean_msg_field(str(row[3]))
                        ...
                        _sid = _cf(raw_sid)
                        _num = _cf(raw_num)
                        if sid_filter and sid_filter not in _sid.lower() and sid_filter not in msg.lower():
                            continue
                        if num_filter and num_filter not in _num:
                            continue
                        items.append({
                            "range":       rng,
                            "number":      _num,
                            ...
                            "received_at": rcv,
                            "account":     target_acc["email"],
                            "source":      "scrape_xhr_test",
                        })
        except Exception as e:
            logger.warning(f"[TEST-SMS] fallback scrape error: {e}")
...
def live_public_sms():
    ...
    # ── Prioritas 2: Scrape langsung (fallback Vercel) ──
    target_acc = next((a for a in ACCOUNTS if not acc_filter or a["email"] == acc_filter), None)
    if not target_acc:
    ...
    if show_raw:
        r = scraper.get(PUBLIC_URL, timeout=15)
        return Response(decode_response(r), mimetype="text/plain; charset=utf-8")

    # ── Scrape DataTables — pakai query variant yang terbukti HTTP 200 ──
    # Field asli dari debug response:
    #   id, termination_id, originator (HTML→SID), messagedata (message),
    #   senttime (received_at), range (range_name),
    #   termination: {id, test_number (HTML→number)}, DT_RowId
    items = []
    try:
        qs = (
            "draw=1"
        ...
        if resp and resp.status_code == 200:
            # ── Helper strip HTML — definisi di luar loop untuk efisiensi ──
            def _clean_field(raw: str) -> str:
                """Strip HTML tags, script blocks, entities, ambil baris pertama bersih."""
                # 1. Decode HTML entities dulu (&amp; &#039; &lt; dll)
                s = html_lib.unescape(raw)
                # 2. Hapus semua <script>...</script> blocks (termasuk multiline)
                s = re.sub(r'<script[\s\S]*?</script>', '', s, flags=re.IGNORECASE)
                # 3. Hapus semua <style>...</style> blocks
                s = re.sub(r'<style[\s\S]*?</style>', '', s, flags=re.IGNORECASE)
                # 4. Hapus semua HTML tags
                s = re.sub(r'<[^>]+>', '', s)
                # 5. Normalize whitespace (tabs, multiple spaces → single space)
                s = re.sub(r'[ \t]+', ' ', s)
                # 6. Ambil hanya baris pertama yang tidak kosong
                for line in s.split('\n'):
                    line = line.strip()
                    if line:
                        return line
            ...
            try:
                ...
                for row in rows:
                    if isinstance(row, dict):
                        ...
                        if isinstance(term, dict):
                            raw_num = str(term.get("test_number", ""))
                        if not raw_num:
                            raw_num = str(row.get("termination_id", ""))
                        msg = str(row.get("messagedata", ""))
                        rcv = str(row.get("senttime", ""))
                    ...
                    sid = _clean_field(raw_sid)
                    num = _clean_field(raw_num)

                    # Clean message: double-decode entities + unicode literals + strip HTML
                ...
                logger.warning(f"[PUBLIC-SMS] JSON parse error: {e}")

    except Exception as e:
        logger.warning(f"[PUBLIC-SMS] scrape error: {e}")

    # ── Fallback akhir: parse HTML tabel langsung ──
    if not items:
        try:
            r    = scraper.get(PUBLIC_URL, timeout=15,
            ...
            for t in soup.find_all("table"):
                ...
                if not any("Range" in h or "SID" in h for h in ths):
                    continue
                for row in t.find_all("tr")[1:limit+1]:
                    cols = [td.get_text(strip=True) for td in row.find_all("td")]
                    if len(cols) < 3 or not any(cols):
                        continue
                    sid = cols[2] if len(cols) > 2 else ""
                    ...
                    items.append({
                        ...
                        "sid":         sid,
                        "message":     msg,
                        "received_at": cols[4] if len(cols) > 4 else "",
                        "source":      "scrape_html",
                        "account":     target_acc["email"],
                    })
                if items:
            ...
            logger.warning(f"[PUBLIC-SMS] HTML scrape error: {e}")

    note = ""
    if not items:
        note = "Data kosong — iVAS pakai obfuscated DataTables. Di VPS akan dapat real-time via WebSocket."
...
@app.route("/debug/public-sms-raw")
