...
# ── Cloudflare bypass — cloudscraper utama, requests fallback ──
try:
...
def build_scraper():
    """
    Buat scraper dengan Cloudflare bypass.
    - cloudscraper: bypass CF Turnstile/JS Challenge otomatis
    - fallback: requests.Session biasa dengan headers realistis
    """
    ...
    if _CLOUDSCRAPER_OK:
        try:
            # cloudscraper — bypass CF Turnstile, JS Challenge, dll
            s = _cloudscraper_mod.create_scraper(
                ...
                delay=3,           # delay 3s sebelum solve challenge
                debug=False,
            )
            # Override UA supaya konsisten
            s.headers.update({
                "User-Agent":      ua,
                "Accept-Language": al,
            ...
            logger.info("[SCRAPER] cloudscraper OK — CF bypass aktif")
            return s
        except Exception as e:
            logger.warning(f"[SCRAPER] cloudscraper gagal ({e}), fallback requests.Session")
    ...
    logger.info("[SCRAPER] requests.Session (tanpa CF bypass)")
    return s
...
def login_account(account):
    ...
    try:
        ...
        # ── GET login page — retry sampai dapat _token (CF challenge bisa muncul) ──
        login_page = None
        tok = None
        for _attempt in range(3):
            ...
            if tok_el and tok_el.get("value"):
                tok = tok_el["value"]
                logger.info(f"[LOGIN] _token OK attempt={_attempt} {email}")
                break
            # Cek apakah kena CF challenge
            if "cf-turnstile" in page_html.lower() or "challenge" in page_html.lower():
                logger.warning(f"[LOGIN] CF challenge detected attempt={_attempt}, retry...")
                time.sleep(2)
                continue
            # Cari _token dari meta CSRF
            meta_tok = soup.find("meta", {"name": "csrf-token"})
            if meta_tok and meta_tok.get("content"):
                tok = meta_tok["content"]
                logger.info(f"[LOGIN] meta _token OK attempt={_attempt} {email}")
                break
            logger.warning(f"[LOGIN] _token tidak ditemukan attempt={_attempt}, html size={len(page_html)}")
            time.sleep(1)

        if not tok:
            return {"ok": False, "error": "_token tidak ditemukan (CF block?)", "email": email}
        ...
        if "/login" in resp_url:
            # Cek apakah masalah CF atau credential salah
            resp_html = decode_response(resp)
            if "cf-turnstile" in resp_html.lower() or "cf_clearance" in resp_html.lower():
                return {"ok": False, "error": "Cloudflare block — coba lagi", "email": email}
