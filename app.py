...
def get_ranges(account, from_date, to_date):
    """
    Level 1 — Ambil daftar range dari iVAS.

    REALITA dari debug output:
      POST /portal/sms/received/getsms dengan from/to → response HTML kosong
      "No SMS found for this date range" + toggleRange(id, safe) = DEFINISI FUNGSI bukan data!

    Flow iVAS yang benar:
      1. GET /portal/sms/received  → halaman utama, sudah embed range list di HTML
         ATAU
      2. POST /portal/sms/received/getsms dengan from/to → dapat HTML berisi div.rng
         tapi ranges dimuat LAZILY — data baru ada setelah user klik

    Solusi: ambil halaman /portal/sms/received langsung (GET),
    parse range dari HTML yang sudah di-render server-side.
    ...
    def _parse_ranges(html, source=""):
        """Parse semua pola range dari satu blok HTML."""
        found = 0

        # Pass 1: onclick="toggleRange('NAMA','ID')" — confirmed pola lama
        for m in re.finditer(r"toggleRange\s*\(\s*'([^']+)'\s*,\s*'([^']+)'\s*\)", html):
            # Pastikan ini bukan definisi fungsi JS (definisi tidak punya string literal sebagai arg)
        ...
        # Pass 3: BeautifulSoup div.rng
        soup = BeautifulSoup(html, "html.parser")
        for div in soup.select("div.rng, div[class*='range-item'], div[class*='rng-item']"):
            rname_el = div.select_one("span.rname, span[class*='range-name'], .rname")
            name     = rname_el.get_text(strip=True) if rname_el else ""
            if not name:
                name = div.get_text(separator="|", strip=True).split("|")[0].strip()
            oc  = div.get("onclick", "")
        ...
        # Pass 4: id="sp_XXXX" pattern — sub-divs yang ada artinya range ada
        for div in soup.select("[id^='sp_']"):
            rid  = div["id"].replace("sp_", "")
            name = rid.replace("_", " ")
            # Cek apakah ada sibling/prev dengan nama range
        ...
        if found > 0:
            logger.info(f"[RANGES] _parse_ranges({source}): {found} entries ditemukan")
        return found

    # ── Attempt 1: GET halaman /portal/sms/received (dengan date params) ──────
    # iVAS render range list di halaman ini langsung
    recv_url_dated = f"{RECV_URL}?from={ivas_from}&to={ivas_to}"
    resp_get, _ = do_request(account, "GET", recv_url_dated,
                             headers={"Referer": BASE_URL, "Accept": "text/html,*/*"})
    if resp_get and resp_get.status_code == 200:
        html_get = decode_response(resp_get)
        _parse_ranges(html_get, "GET-dated")
        logger.info(f"[RANGES] GET dated → {len(result)} ranges")

    # ── Attempt 2: GET halaman /portal/sms/received (tanpa filter) ────────────
    if not result:
        resp_plain, _ = do_request(account, "GET", RECV_URL,
                                   headers={"Referer": BASE_URL, "Accept": "text/html,*/*"})
        if resp_plain and resp_plain.status_code == 200:
            html_plain = decode_response(resp_plain)
            _parse_ranges(html_plain, "GET-plain")
            logger.info(f"[RANGES] GET plain → {len(result)} ranges")

    # ── Attempt 3: POST /portal/sms/received/getsms (cara lama, fallback) ─────
    if not result:
        resp_post, _ = do_request(
            account, "POST",
        ...
        if resp_post and resp_post.status_code == 200:
            html_post = decode_response(resp_post)
            # Skip kalau response adalah "No SMS found" empty state
            if "sms-empty" not in html_post and "No SMS found" not in html_post:
                _parse_ranges(html_post, "POST-getsms")
            else:
                logger.info(f"[RANGES] POST getsms → 'No SMS found' response, skip")

    # ── Attempt 4: POST dengan format date berbeda ─────────────────────────────
    if not result:
        # Coba format date tanpa konversi (DD/MM/YYYY langsung)
        resp_post2, _ = do_request(
            account, "POST",
            f"{BASE_URL}/portal/sms/received/getsms",
            data={"from": from_date, "to": to_date},
            headers=ajax_hdrs(),
        )
        if resp_post2 and resp_post2.status_code == 200:
            html_post2 = decode_response(resp_post2)
            if "sms-empty" not in html_post2 and "No SMS found" not in html_post2:
                _parse_ranges(html_post2, "POST-getsms-raw-date")

    # ── Attempt 5: GET dengan query string format iVAS (start/end) ────────────
    if not result:
        resp_get2, _ = do_request(
            account, "GET",
            f"{RECV_URL}?start={ivas_from}&end={ivas_to}",
            headers={"Referer": BASE_URL},
        )
        if resp_get2 and resp_get2.status_code == 200:
            html_get2 = decode_response(resp_get2)
            _parse_ranges(html_get2, "GET-start-end")

    if not result:
        logger.warning(f"[RANGES] 0 ranges setelah 5 attempt. Date: {from_date}")
    else:
        logger.info(f"[RANGES] FINAL {len(result)} ranges: {[(r['name'], r['id']) for r in result]}")
    return result
...
def get_sms(account, phone_number, range_name, from_date, to_date):
    ...
    # ── Pass 4: table tanpa header (cari kolom paling panjang) ───────────
    for tbl in soup.find_all("table"):
        rows = tbl.find_all("tr")
        for tr in rows:
            tds = tr.find_all("td")
            # Kolom message biasanya yang terpanjang (lebih dari 10 char)
            candidates = []
            for td in tds:
                t = _clean(td.get_text(separator=" ", strip=True))
                if len(t) > 10 and not t.isdigit():
                    candidates.append(t)
            # Pilih yang paling panjang
            if candidates:
                best = max(candidates, key=len)
                if len(best) > 10:
                    logger.info(f"[SMS] {phone_number} ✓ tabel longest-col")
                    return best
...
def debug_sms_deep():
    ...
    # ── STEP 1: GET RANGES ────────────────────────────────────────────────────
    out.append("\n" + SEP)
    out.append("  STEP 1 -- GET/POST ambil RANGES  [5 ATTEMPT]")
    out.append(SEP)
    out.append("\n  iVAS pakai lazy-load — ranges ada di halaman received, BUKAN POST getsms!")
    out.append("  Coba 5 cara sekaligus untuk debug:\n")

    ivas_date_end = to_ivas_date(request.args.get("to_date", date_str))

    # Attempt 1: GET dated
    r1a, b1a = _raw_post.__wrapped__(acc, "GET",
        RECV_URL + "?from=" + ivas_date + "&to=" + ivas_date_end, {}) \
        if hasattr(_raw_post, "__wrapped__") else (None, "")

    # Langsung pakai do_request untuk semua attempt
    def _do_get(url):
        scraper = get_session(acc, force=False)
        if not scraper or not scraper.get("ok"):
            return None, "SESSION FAIL"
        try:
            r = scraper["scraper"].get(url, headers={"Referer": BASE_URL,
                "Accept": "text/html,application/xhtml+xml,*/*"}, timeout=20)
    ...
    attempts_info = [
        ("Attempt 1 — GET /portal/sms/received?from=..&to=.. (MAIN FIX)",
         "GET", RECV_URL + "?from=" + ivas_date + "&to=" + ivas_date),
        ("Attempt 2 — GET /portal/sms/received (tanpa filter)",
         "GET", RECV_URL),
        ("Attempt 3 — POST /portal/sms/received/getsms (cara lama)",
         "POST_GETSMS", BASE_URL + "/portal/sms/received/getsms"),
        ("Attempt 4 — POST getsms DD/MM/YYYY langsung",
         "POST_RAW", BASE_URL + "/portal/sms/received/getsms"),
        ("Attempt 5 — GET ?start=..&end=..",
         "GET", RECV_URL + "?start=" + ivas_date + "&end=" + ivas_date),
    ]

    for (label, method, url) in attempts_info:
        out.append("\n  [" + label + "]")
        out.append("    URL: " + url)
        if method == "GET":
            rx, bx = _do_get(url)
        elif method == "POST_GETSMS":
            rx, bx = _do_post(url, {"from": ivas_date, "to": ivas_date})
        else:  # POST_RAW
            rx, bx = _do_post(url, {"from": date_str, "to": date_str})

        status  = str(rx.status_code) if rx else "NULL"
        final_u = getattr(rx, "url", "?") if rx else "?"
        is_empty = "sms-empty" in bx or "No SMS found" in bx
        tr_raw  = re.findall(r"toggleRange\s*\([^)]+\)", bx)
        # Filter definisi fungsi — definisi pakai identifier, bukan string literal
        tr_data = re.findall(r"toggleRange\s*\(\s*['\"]([^'\"]+)['\"]", bx)
        sp_ids  = re.findall(r'id=["\']sp_([^"\']+)["\']', bx)
        div_rng = len(BeautifulSoup(bx, "html.parser").select("div.rng")) if bx else 0

        out.append("    HTTP        : " + status)
        out.append("    Final URL   : " + final_u)
        out.append("    Body Len    : " + str(len(bx)) + " chars")
        out.append("    Is Empty    : " + ("YA - No SMS found" if is_empty else "tidak"))
        out.append("    toggleRange() total    : " + str(len(tr_raw)) + " (termasuk definisi fungsi)")
        out.append("    toggleRange() DATA     : " + str(len(tr_data)) + " (string literal = range nyata)")
        for td in tr_data[:5]:
            out.append("      -> '" + td + "'")
        out.append("    div.rng count  : " + str(div_rng))
        out.append("    id='sp_*' found: " + str(len(sp_ids)) + " -> " + str(sp_ids[:5]))
        out.append("    Body 400 char  :")
        out.append("    " + bx[:400].replace("\n", "\n    "))
        out.append("    ...")

    # Hasil final get_ranges
    ranges = get_ranges(acc, date_str, date_str)
    out.append("\n  FINAL get_ranges() -> " + str(len(ranges)) + " ranges:")
    for rng in ranges:
        marker = "  <-- [FILTER MATCH]" if filter_rng and filter_rng.lower() in rng["name"].lower() else ""
        out.append("    name='" + rng["name"] + "'  id='" + rng["id"] + "'" + marker)

    if not ranges:
        out.append("\n  STOP: Tidak ada range. Periksa output attempt di atas.")
        out.append("  Kalau Attempt 1 dapat Body tapi tidak ada toggleRange DATA -> iVAS pakai struktur baru.\n")
        return Response("\n".join(out), mimetype="text/plain; charset=utf-8")
