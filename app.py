...
def live_my_sms():
    ...
    def _scrape_my_sms(acc):
        """Scrape received SMS dari 1 akun via /portal/sms/received/getsms."""
        result = []
        try:
            sess = get_session(acc)
            ...
            today     = datetime.now().strftime("%Y-%m-%d")  # iVAS pakai yyyy-mm-dd

            for date_str in [today]:  # bisa extend ke hari sebelumnya kalau perlu
                recv_data = {"_token": recv_csrf, "from": date_str, "to": date_str}
                hdrs_recv = {
                    "Accept":           "application/json, text/javascript, */*; q=0.01",
                    "X-Requested-With": "XMLHttpRequest",
                ...
                try:
                    ...
                    for rrow in rrows[:limit]:
                        rng = _ivas_clean_sid(str(rrow.get("range", rrow.get("range_name", ""))))
                        num = _ivas_clean_sid(str(rrow.get("number", rrow.get("test_number", ""))))
                        sid = _ivas_clean_sid(str(rrow.get("sid", rrow.get("originator", ""))))
                        msg = _ivas_clean_msg(str(rrow.get("message", rrow.get("messagedata", ""))))
                    ...
                    logger.warning(f"[MY-SMS] parse {acc['email'][:20]}: {ep}")
...
def live_my_sms_poll():
    ...
    def _poll_one(acc):
        ...
        try:
            ...
            rrows = rdata if isinstance(rdata, list) else rdata.get("data", [])
            for rrow in rrows:
                rcv = str(rrow.get("received_at", rrow.get("senttime", "")))
                if rcv and rcv <= since:
                    continue  # skip yang lama
                sid = _ivas_clean_sid(str(rrow.get("sid", rrow.get("originator", ""))))
                msg = _ivas_clean_msg(str(rrow.get("message", rrow.get("messagedata", ""))))
                if sid_filter and sid_filter not in sid.lower() and sid_filter not in msg.lower():
                    continue
                result.append({
                    "range":       _ivas_clean_sid(str(rrow.get("range", ""))),
                    "number":      _ivas_clean_sid(str(rrow.get("number", ""))),
                    "sid":         sid,
                    "message":     msg,
                    "account":     acc["email"],
                    "received_at": rcv,
