...
def live_public_sms():
    ...
    # Scrape via XHR — iVAS kirim JSON kalau ada header X-Requested-With
    items = []
    try:
        # Coba berbagai variasi kolom yang mungkin dipakai iVAS
        col_variants = [
            # Variant 1: nama kolom dari header tabel
            (["range_name","test_number","sid","message_content","received_time"],
             ["range_name","test_number","sid","message_content","received_time"]),
            # Variant 2: nama snake_case alternatif
            (["range","number","sid","message","received_at"],
             ["range","number","sid","message","received_at"]),
            # Variant 3: nama dari iVAS lain yang sudah proven
            (["termination","cli","A2P","message","created_at"],
             ["termination","cli","A2P","message","created_at"]),
        ]

        for col_data, col_name in col_variants:
            rows, total = _fetch_datatables(
                target_acc, PUBLIC_URL,
                length=limit, col_data=col_data, col_name=col_name,
                fallback_fields=col_data,
            )
            if rows:
                for row in rows:
                    # Coba semua kemungkinan key nama kolom
                    rng = str(row.get("range_name", row.get("range",
                              row.get("termination", "")))).strip()
                    num = str(row.get("test_number", row.get("number",
                              row.get("cli", "")))).strip()
                    sid = str(row.get("sid", row.get("A2P", ""))).strip()
                    msg = str(row.get("message_content", row.get("message", ""))).strip()
                    rcv = str(row.get("received_time", row.get("received_at",
...
def debug_public_sms_raw():
    ...
    for i, qs in enumerate(variants):
        ...
        try:
            ...
            out.append(f"Body (500 chars): {body[:500]}")
        except Exception as e:
            out.append(f"Error: {e}")
        out.append("")
