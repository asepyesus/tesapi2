...
@app.route("/debug/login-html")
def debug_login_html():
    """Dump full HTML iVAS login page."""
    try:
        sc = build_scraper()
        r  = sc.get(LOGIN_URL, timeout=20)
        h  = decode_response(r)
        info = [
            f"Status : {r.status_code}",
            f"Size   : {len(h)}",
            f"CF-Ray : {r.headers.get('cf-ray','none')}",
            "===FULL HTML===",
            h
        ]
        return Response("\n".join(info), mimetype="text/plain; charset=utf-8")
    except Exception as e:
