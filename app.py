...
# ── Cloudflare Turnstile config (CONFIRMED dari HTML iVAS) ──
_IVAS_SITEKEY   = "0x4AAAAAACqVmW6ncA-jc1oz"  # CONFIRMED dari debug HTML
...
def _solve_turnstile(sitekey=None, page_url=None):
    """Solve Cloudflare Turnstile via CapSolver API."""
    sitekey  = sitekey or _IVAS_SITEKEY
    page_url = page_url or LOGIN_URL
    if not _CAPSOLVER_KEY:
        logger.warning("[TURNSTILE] CAPSOLVER_KEY tidak ada")
        return ""
    try:
        logger.info(f"[TURNSTILE] Solving sitekey={sitekey[:30]}...")
        r1 = requests.post(
        ...
        if not task_id:
            logger.error(f"[TURNSTILE] createTask error: {d1}")
            return ""
        logger.info(f"[TURNSTILE] taskId={task_id} polling...")
        for attempt in range(30):
            time.sleep(2)
            r2 = requests.post(
                "https://api.capsolver.com/getTaskResult",
                json={"clientKey": _CAPSOLVER_KEY, "taskId": task_id},
                timeout=15)
            d2 = r2.json()
            st = d2.get("status", "")
            if st == "ready":
                ...
                if token:
                    logger.info(f"[TURNSTILE] SOLVED attempt={attempt+1} token={token[:40]}...")
                    return token
                logger.error(f"[TURNSTILE] ready tapi token kosong: {d2}")
                return ""
            if st == "processing":
                continue
            logger.error(f"[TURNSTILE] Error: {d2.get('errorCode')} {d2.get('errorDescription')}")
            return ""
        logger.error("[TURNSTILE] Timeout 60s")
        return ""
...
def login_account(account):
    ...
    try:
        ...
        # ── STEP 2: Solve Turnstile (sitekey CONFIRMED 0x4AAAAAACqVmW6ncA-jc1oz) ──
        turnstile_token = _solve_turnstile(_IVAS_SITEKEY, LOGIN_URL)
        if turnstile_token:
            logger.info(f"[LOGIN] Turnstile SOLVED {email}")
        else:
            logger.warning(f"[LOGIN] Turnstile solve GAGAL {email}, coba tanpa token...")
        ...
        if "/login" in resp_url:
            ...
            if "turnstile" in resp_html.lower():
                return {"ok": False,
                        "error": "Turnstile verify gagal — cek saldo CapSolver",
                        "email": email}
...
def debug_cf_login():
    ...
    try:
        ...
        out.append(f"sitekey    : {_IVAS_SITEKEY} (hardcoded)")
        out.append(f"CapSolver  : {_CAPSOLVER_KEY[:25]}...")
        out.append("")
        ...
        # Solve Turnstile
        out.append("Solving Turnstile...")
        ts_token = _solve_turnstile(_IVAS_SITEKEY, LOGIN_URL)
        out.append(f"Turnstile  : {'SOLVED: '+ts_token[:50]+'...' if ts_token else 'FAILED!'}")
        out.append("")
