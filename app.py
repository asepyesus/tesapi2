...
def live_my_sms():
    ...
    for t in tables:
        ...
        if "sid" in headers or "paid" in headers:
            for row in t.find_all("tr")[1:limit+1]:
                ...
                if len(cols) >= 3 and any(c for c in cols):
                    items.append({
                        ...
                        "limit":       cols[4] if len(cols) > 4 else "",
                        "message":     cols[5] if len(cols) > 5 else "",
                        "account":     target_acc["email"],
                        "received_at": datetime.now().isoformat(),
                        "source":      "scrape_my_sms",
                    })
            break
    ...
    return jsonify({
        ...
        "total":   len(items),
        "sms":     items,
        "note":    "Data dari scrape HTML (WebSocket cache kosong)" if not items else "",
    })
...
def live_test_sms():
    ...
    # Bersihkan field raw yang terlalu besar
    clean = []
    for s in items:
        clean.append({
            "range":       s.get("range", ""),
            "number":      s.get("phone_number", ""),
            "sid":         s.get("sid", ""),
            "message":     s.get("otp_message", ""),
            "account":     s.get("account", ""),
            "received_at": s.get("received_at", ""),
            "source":      s.get("source", ""),
        })
...
def live_public_sms():
    """
    Public SMS History — data dari DataTables AJAX /portal/sms/test/sms.
    iVAS pakai jQuery DataTables, data di-load via AJAX POST/GET.

    Query params:
      account=email   → akun yang dipakai untuk scrape
      limit=N         → max hasil (default: 50, max: 500)
      sid=X           → filter SID/service (case-insensitive)
      range=X         → filter range name
      raw=1           → raw HTML halaman (debug)
      ajax=1          → debug raw AJAX response

    Response JSON:
      {status, total, sms: [{range_name, number, sid, message, received_at}]}
    """
    PUBLIC_HTML_URL = f"{BASE_URL}/portal/sms/test/sms"
    acc_filter = request.args.get("account", "").strip()
    limit      = min(int(request.args.get("limit", 50)), 500)
    sid_filter = request.args.get("sid",   "").strip().lower()
    rng_filter = request.args.get("range", "").strip().lower()
    show_raw   = request.args.get("raw",   "0") == "1"
    show_ajax  = request.args.get("ajax",  "0") == "1"
    ...
    if show_raw:
        r = scraper.get(PUBLIC_HTML_URL, timeout=15)
        return Response(decode_response(r), mimetype="text/plain; charset=utf-8")

    items = []

    # ── DataTables GET — pakai fungsi _fetch_datatables yang sudah proven ──
    # iVAS /portal/sms/test/sms hanya support GET (405 untuk POST)
    # Kolom: Range Name, Test Number, SID, Message content, Received time
    col_data = ["range_name", "test_number", "sid", "message_content", "received_time"]
    col_name = ["range_name", "test_number", "sid", "message_content", "received_time"]
    fallback = ["range_name", "test_number", "sid", "message_content", "received_time"]
    ...
    if show_ajax:
        # Debug: tampilkan raw GET request
        dt_qs = (
            f"draw=1&columns[0][data]=range_name&columns[0][name]=range_name"
            f"&columns[1][data]=test_number&columns[1][name]=test_number"
            f"&columns[2][data]=sid&columns[2][name]=sid"
            f"&columns[3][data]=message_content&columns[3][name]=message_content"
            f"&columns[4][data]=received_time&columns[4][name]=received_time"
            f"&order[0][column]=4&order[0][dir]=desc"
            f"&start=0&length={limit}&search[value]=&search[regex]=false"
        )
        sess2 = get_session(target_acc)
        scraper2 = sess2["scraper"] if sess2 else sess["scraper"]
        try:
            r_dbg = scraper2.get(
                f"{PUBLIC_HTML_URL}?{dt_qs}",
                headers={"Accept": "application/json, text/javascript, */*; q=0.01",
                         "X-Requested-With": "XMLHttpRequest",
            ...
            return Response(
                f"GET {PUBLIC_HTML_URL}\nStatus: {r_dbg.status_code}\n\n{decode_response(r_dbg)}",
    ...
    if rows:
        ajax_ok = True
        for row in rows:
            rng  = str(row.get("range_name",      row.get("range", ""))).strip()
            num  = str(row.get("test_number",      row.get("number", ""))).strip()
            sid  = str(row.get("sid",              row.get("SID", ""))).strip()
            msg  = str(row.get("message_content",  row.get("message", ""))).strip()
            rcvd = str(row.get("received_time",    row.get("received_at", ""))).strip()

            # Bersihkan HTML tags kalau ada
            rng  = re.sub(r'<[^>]+>', '', rng).strip()
            ...
            items.append({
                ...
                "message":     msg,
                "received_at": rcvd,
                "source":      "datatables_get",
                "account":     target_acc["email"],
            })

    # ── Fallback: scrape HTML tabel ──
    if not ajax_ok:
        try:
            r    = scraper.get(PUBLIC_HTML_URL, timeout=15)
            html = decode_response(r)
            ...
            for t in soup.find_all("table"):
                headers = [th.get_text(strip=True).lower() for th in t.find_all("th")]
                if "range name" in headers or ("sid" in headers and "range" in " ".join(headers)):
                    for row in t.find_all("tr")[1:limit+1]:
                        cols = [td.get_text(strip=True) for td in row.find_all("td")]
                        if len(cols) >= 3 and any(c for c in cols):
                            sid = cols[2] if len(cols) > 2 else ""
                            msg = cols[3] if len(cols) > 3 else ""
                            rng = cols[0] if len(cols) > 0 else ""
                            if sid_filter and sid_filter not in sid.lower() and sid_filter not in msg.lower():
                                continue
                            if rng_filter and rng_filter not in rng.lower():
                                continue
    ...
    return jsonify({
        "status":  "success",
        "source":  "datatables_ajax" if ajax_ok else "scrape_html",
        "account": target_acc["email"],
        "total":   len(items),
        "sms":     items,
    })
...
# ════════════════════════════════════════════════════════
# APP STARTUP — Init WebSocket setelah app ready
# ════════════════════════════════════════════════════════
