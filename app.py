...
def live_public_sms():
    ...
    # ── Coba DataTables AJAX endpoint ──
    # iVAS DataTables biasanya POST ke URL yang sama dengan parameter draw, start, length
    dt_params = {
        "draw":             "1",
        "start":            "0",
        "length":           str(limit),
    ...
    for method in ["POST", "GET"]:
        try:
            if method == "POST":
                r = scraper.post(PUBLIC_HTML_URL, data=dt_params,
                                 headers=dt_headers, timeout=15)
            else:
                r = scraper.get(PUBLIC_HTML_URL,
            ...
            if show_ajax:
                return Response(f"Method: {method}\nStatus: {r.status_code}\n\n{body}",
                                mimetype="text/plain; charset=utf-8")

            # Coba parse JSON DataTables response
            try:
                import json as _json
                # Bersihkan prefix angka kalau ada (Socket.IO style)
                clean_body = re.sub(r'^\d+', '', body.strip())
                jdata = _json.loads(clean_body)
                ...
                if rows:
                    for row in rows[:limit]:
                        if isinstance(row, list) and len(row) >= 4:
                            rng  = re.sub(r'<[^>]+>', '', str(row[0])).strip()
                            num  = re.sub(r'<[^>]+>', '', str(row[1])).strip()
                            sid  = re.sub(r'<[^>]+>', '', str(row[2])).strip()
                            msg  = re.sub(r'<[^>]+>', '', str(row[3])).strip()
                            rcvd = re.sub(r'<[^>]+>', '', str(row[4])).strip() if len(row) > 4 else ""
                        elif isinstance(row, dict):
                            rng  = str(row.get("range_name", row.get("range", "")))
                            num  = str(row.get("test_number", row.get("number", "")))
                            sid  = str(row.get("sid", row.get("SID", "")))
                            msg  = str(row.get("message_content", row.get("message", "")))
                            rcvd = str(row.get("received_time", row.get("received_at", "")))
                        else:
                            continue
            ...
            logger.warning(f"[PUBLIC-SMS] {method} error: {e}")

    # ── Fallback: scrape HTML tabel (rows mungkin 0 karena DataTables lazy load) ──
    if not ajax_ok:
        try:
            r    = scraper.get(PUBLIC_HTML_URL, timeout=15)
            html = decode_response(r)
