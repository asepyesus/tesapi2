...
def _scrape_jwt_token(scraper, page_url: str) -> str | None:
    ...
    try:
        ...
        # ── Pattern 1a: Full JWT 3-bagian (Header.Payload.Signature) ──
        all_jwts = re.findall(
        ...
        if all_jwts:
            tok = all_jwts[0]
            logger.info(f"[WS-JWT] ✅ JWT 3-bagian dari HTML: {tok[:30]}...")
            return tok

        # ── Pattern 1b: Laravel/iVAS session token format eyJ (tanpa titik) ──
        # iVAS kirim token format: eyJpdi6Im... di console.log atau _token
        # Ini bukan JWT standard tapi dipakai Socket.IO iVAS sebagai auth
        all_eyj = re.findall(
            r'(?:console\.log\(|["\'])( eyJ[A-Za-z0-9+/=_\-]{30,})(?:["\']|\))',
            html
        )
        if not all_eyj:
            # Coba cari di value attribute atau assignment
            all_eyj = re.findall(
                r'(?:token|_token|jwt|auth)["\s]*[:=]["\s]*(eyJ[A-Za-z0-9+/=_\-]{30,})',
                html,
                re.IGNORECASE,
            )
        if not all_eyj:
            # Fallback: ambil semua string panjang yang mulai eyJ (minimal 80 char)
            all_eyj = re.findall(
        ...
        if all_eyj:
            tok = all_eyj[0].strip()
            logger.info(f"[WS-JWT] ✅ eyJ token dari HTML (non-JWT): {tok[:30]}...")
            return tok
    ...
    if accounts:
        ...
        if sess and sess.get("ok"):
            ...
            try:
                ...
                # Coba extract semua eyJ tokens (non-JWT format juga)
                out.append("")
                out.append("[ TOKEN KANDIDAT dari HTML ]")
                # 3-bagian JWT standard
                jwts_3 = re.findall(
                    r'\b(eyJ[A-Za-z0-9_\-]{10,}\.[A-Za-z0-9_\-]{10,}\.[A-Za-z0-9_\-]{10,})\b', html)
                # Token panjang eyJ (Laravel/iVAS format)
                eyj_long = re.findall(r'(eyJ[A-Za-z0-9+/=_\-]{80,})', html)
                # Dari console.log
                eyj_console = re.findall(
                    r'console\.log\(["\']?(eyJ[A-Za-z0-9+/=_\-]{30,})["\']?\)', html)
                if jwts_3:
                    out.append(f"  ✅ JWT 3-bagian: {jwts_3[0][:60]}...")
                if eyj_console:
                    out.append(f"  ✅ console.log token: {eyj_console[0][:60]}...")
                if eyj_long and not jwts_3 and not eyj_console:
                    out.append(f"  ✅ eyJ panjang: {eyj_long[0][:60]}...")
                if not jwts_3 and not eyj_console and not eyj_long:
                    out.append("  ❌ Tidak ada token eyJ ditemukan di HTML")
            except Exception as e:
                out.append(f"  Error: {e}")
        else:
            out.append("  ❌ Login gagal")
