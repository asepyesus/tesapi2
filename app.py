...
def live_public_sms():
    """
    Public SMS — real-time dari WebSocket event send_message_max_Limit_217253.
    iVAS kirim event ini untuk semua SMS public (halaman /portal/sms/test/sms).
    DataTable di halaman itu pakai obfuscated JS — tidak bisa di-scrape langsung.

    Query params:
      limit=N         → max hasil (default: 100, max: 500)
      sid=X           → filter SID/service (case-insensitive)
      range=X         → filter range name
      account=email   → filter per akun pengirim event
      raw=1           → raw HTML halaman (debug)

    Response JSON:
      {status, source, total, sms: [{range_name, number, sid, message, account, received_at}]}
    """
    ...
    if show_raw:
        ...
        if target_acc:
            sess = get_session(target_acc)
            if sess and sess.get("ok"):
                r = sess["scraper"].get(f"{BASE_URL}/portal/sms/test/sms", timeout=15)
                return Response(decode_response(r), mimetype="text/plain; charset=utf-8")
        return Response("Login gagal", mimetype="text/plain"), 500

    # ── Ambil dari WebSocket public cache ──
    items_raw = get_ws_public(limit=limit * 2, sid_filter=sid_filter, range_filter=rng_filter)

    # Filter per akun kalau diminta
    if acc_filter:
        items_raw = [i for i in items_raw if acc_filter in i.get("account", "")]

    clean = []
    for s in items_raw[:limit]:
        clean.append({
            ...
            "account":     s.get("account", ""),
            "received_at": s.get("received_at", ""),
            "source":      s.get("source", "websocket_public"),
        })

    ws_total = sum(len(q) for q in _ws_public_cache.values())
    ...
    return jsonify({
        ...
        "cached":     ws_total,
        "sms":        clean,
        "note":       "Real-time via WebSocket event send_message_max_Limit_217253" if clean
                      else "Cache kosong — tunggu SMS masuk atau hit /ws/reconnect",
    })
