...
def debug_sms_deep():
    ...
    out.append(SEP)
    out.append("\n  Payload: from=" + ivas_date + "  to=" + ivas_date + "  _token=recv_csrf\n")

    r1, b1 = _raw_post(acc,
        BASE_URL + "/portal/sms/received/getsms",
        {"from": ivas_date, "to": ivas_date},
    )
    out.append("  HTTP Status  : " + (str(r1.status_code) if r1 else "NULL"))
    out.append("  Final URL    : " + (getattr(r1, "url", "?") if r1 else "NULL"))
    out.append("  Content-Type : " + (r1.headers.get("Content-Type", "?") if r1 else "?"))
    out.append("  Body Length  : " + str(len(b1)) + " chars")
    out.append("  Is Login Pg  : " + ("YA <- session gagal" if r1 and "/login" in getattr(r1, "url", "") else "tidak"))

    tr_matches = re.findall(r"toggleRange\s*\([^)]+\)", b1)
    out.append("\n  toggleRange() ditemukan: " + str(len(tr_matches)))
    for tm in tr_matches[:10]:
        out.append("    " + tm)
    if len(tr_matches) > 10:
        out.append("    ... (+" + str(len(tr_matches) - 10) + " lagi)")

    ranges = get_ranges(acc, date_str, date_str)
    out.append("\n  Parsed ranges (" + str(len(ranges)) + "):")
    for rng in ranges:
        marker = "  <-- [FILTER MATCH]" if filter_rng and filter_rng.lower() in rng["name"].lower() else ""
        out.append("    name='" + rng["name"] + "'  id='" + rng["id"] + "'" + marker)

    out.append("\n" + SEP2)
    out.append("  RAW HTML Step 1 (500 char pertama)")
    out.append(SEP2)
    out.append(b1[:500])
    out.append("... [truncated — /debug/ranges-raw untuk full]\n")

    if not ranges:
        out.append("STOP: Tidak ada range. Cek CSRF atau tidak ada SMS hari ini.\n")
        return Response("\n".join(out), mimetype="text/plain; charset=utf-8")
    ...
    if filter_rng:
        ...
        if not working_ranges:
            out.append("Range '" + filter_rng + "' tidak ditemukan, pakai range pertama.\n")
