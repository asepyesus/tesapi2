...
def add_number():
    ...
    Bisa pakai range_name (add semua nomor di range itu) ATAU termination_id/number spesifik.
    Support multi-akun paralel — kalau account kosong, eksekusi ke SEMUA akun.

    CONFIRMED dari JS iVAS:
      function AddNumbers(id) {
          $.ajax({ url: ".../portal/numbers/termination/number/add",
                   type: "post", data: { _token: "...", id: id } })
      }
      → Hanya butuh 'id' (termination_id)

    Params:
      range_name     : nama range, misal "PAKISTAN 34" → add SEMUA nomor di range ini
      termination_id : ID spesifik dari Test Numbers (opsional, alternatif range_name)
      number         : nomor telepon spesifik → auto-resolve ke termination_id (opsional)
      account        : email akun (opsional) → kalau kosong: semua akun paralel
      limit          : max nomor per akun kalau pakai range_name (default 500)
      dry_run        : "1" → preview saja, tidak eksekusi (default "0")

    Contoh:
      /numbers/add?range_name=PAKISTAN 34                          ← add semua nomor di range, semua akun
      /numbers/add?range_name=PAKISTAN 34&account=email@x.com     ← 1 akun saja
      /numbers/add?range_name=PAKISTAN 34&dry_run=1               ← preview dulu
      /numbers/add?termination_id=82774                           ← 1 nomor spesifik, semua akun
      /numbers/add?number=923008264692                            ← by nomor telepon
    """
    ...
    if not range_name and not termination_id and not number:
        return jsonify({
            ...
            "tip":      "Pakai range_name untuk add semua nomor dalam 1 range ke semua akun",
        }), 400
    ...
    def _process_account(session):
        ...
        items = []  # list of {"termination_id": str, "number": str}

        if range_name:
            # Fetch semua nomor di range ini untuk akun ini
            try:
                rows, _ = _fetch_datatables(
                    account, f"{BASE_URL}/portal/numbers/test",
                    search=range_name, length=limit
                )
                for row in rows:
                    rng = re.sub(r"<[^>]+>","",str(row.get("range",""))).strip()
                    if rng.lower() != range_name.lower():
                        continue
                    # Prioritas: id > DT_RowId > action button
        ...
        # ── Dry run: hanya list, tidak eksekusi ───────────────────────────────
        if dry_run:
            with lock:
                all_previews.append({
                    "account": email,
                    "found":   len(items),
                    "numbers": items[:20],  # preview max 20
                })
            return

        # ── Eksekusi add satu per satu ────────────────────────────────────────
        logger.info(f"[ADD] {email}: {len(items)} nomor akan di-add (range='{range_name}')")
        for item in items:
            ...
            try:
                ...
                with lock:
                    if success:
                        ...
                        logger.warning(f"[ADD] {email}: stop at tid={tid}: {message}")
                        break  # Rate limit / akun penuh → stop akun ini
                    else:
                        all_errors.append(entry)
                logger.info(f"[ADD] {email}: tid={tid} {'✅' if success else '❌'} {message}")
                _time.sleep(0.3)  # hindari rate limit
            except Exception as e:
                with lock:
                    all_errors.append({
                        "account": email, "termination_id": tid, "number": num,
    ...
    if dry_run:
        ...
        return jsonify({
            ...
            "tip":        "Hapus &dry_run=1 untuk eksekusi add",
        })
    ...
    return jsonify({
        ...
        "accounts_fail": len(set(e["account"] for e in all_errors if "account" in e)),
        ...
        "tip":           "Jika skipped karena rate limit, tunggu beberapa menit lalu coba lagi",
...
def add_numbers_by_range():
    """
    Alias untuk /numbers/add?range_name=...
    Tambah SEMUA nomor dalam 1 range dari Test Numbers ke My Numbers — semua akun paralel.
    ...
    Params:
      ...
      dry_run    : (opsional) "1" → preview saja (default "0")
    ...
    """
    # Forward ke add_number dengan request yang sama
    from flask import request as _req
    # Inject range_name ke args kalau belum ada
    return add_number()
