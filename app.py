...
def debug_login_flow():
    ...
    # ── STEP 6: FULL RAW HTML DUMP semua endpoint AJAX ──
    if step_filter in ("all", "6"):
        out.append("STEP 6: RAW HTML DUMP — semua endpoint AJAX iVAS")
        out.append(SEP2)
        try:
            sess = get_session(target_acc)
            if not sess.get("ok"):
                out.append("ERROR: Session tidak tersedia, jalankan step 2 dulu")
            else:
                sc   = sess["scraper"]
                today_s = datetime.now().strftime("%Y-%m-%d")

                # ── 6A: GET /portal/sms/received (shell page) ──
                out.append("[ 6A ] GET /portal/sms/received — shell page (raw)")
                out.append(SEP2)
                try:
                    r6a  = sc.get(RECV_URL, timeout=15)
                    h6a  = decode_response(r6a)
                    out.append(f"Status : {r6a.status_code}")
                    out.append(f"Size   : {len(h6a)} chars")
                    out.append(f"URL    : {r6a.url}")
                    out.append("")
                    out.append("[ RAW HTML (pertama 3000 chars) ]")
                    out.append(h6a[:3000])
                    out.append("...")
                    out.append("")
                    out.append("[ RAW HTML (chars 3000-6000) ]")
                    out.append(h6a[3000:6000])
                    out.append("...")
                    out.append("")
                    out.append("[ SEMUA <script> inline (no src) ]")
                    soup6a = BeautifulSoup(h6a, "html.parser")
                    for i, sc_tag in enumerate(soup6a.find_all("script")):
                        if not sc_tag.get("src"):
                            txt = sc_tag.get_text().strip()
                            if txt:
                                out.append(f"--- script #{i+1} ({len(txt)} chars) ---")
                                out.append(txt[:2000])
                    out.append("")
                    out.append("[ SEMUA external <script src> ]")
                    for sc_tag in soup6a.find_all("script", src=True):
                        out.append(f"  {sc_tag['src']}")
                    out.append("")
                    out.append("[ SEMUA <form> tags ]")
                    for fm in soup6a.find_all("form"):
                        out.append(str(fm)[:600])
                        out.append("---")
                except Exception as e:
                    out.append(f"ERROR 6A: {e}")
                out.append(SEP)

                # ── 6B: POST /portal/sms/received/getsms (AJAX ranges) ──
                out.append("[ 6B ] POST /portal/sms/received/getsms — ranges fragment (raw)")
                out.append(SEP2)
                try:
                    # Ambil fresh CSRF dari halaman received
                    csrf6b = _get_csrf_cached(sc, RECV_URL) or sess.get("recv_csrf","")
                    post_data6b = {
                        "_token": csrf6b,
                        "from":   today_s,
                        "to":     today_s,
                    ...
                    r6b = sc.post(f"{BASE_URL}/portal/sms/received/getsms",
                                  data=post_data6b, headers=test_hdrs6b, timeout=15)
                    h6b = decode_response(r6b)
                    out.append(f"Status       : {r6b.status_code}")
                    out.append(f"Content-Type : {r6b.headers.get('Content-Type','')}")
                    out.append(f"Size         : {len(h6b)} chars")
                    out.append(f"CSRF used    : {csrf6b[:40] if csrf6b else 'MISSING'}")
                    out.append(f"Params       : from={today_s} to={today_s}")
                    out.append("")
                    out.append("[ FULL RAW RESPONSE ]")
                    out.append(h6b)
                    out.append("")
                    out.append("[ toggleRange() calls found ]")
                    for m in re.finditer(r"toggleRange\s*\(['\"]([^'\"]+)['\"],\s*['\"]([^'\"]+)['\"]", h6b):
                        out.append(f"  name={m.group(1)} id={m.group(2)}")
                    out.append("")
                    out.append("[ div.rng elements ]")
                    soup6b = BeautifulSoup(h6b, "html.parser")
                    for div in soup6b.select("div.rng, div[onclick*='toggleRange']"):
                        out.append(str(div)[:300])
                        out.append("---")
                except Exception as e:
                    out.append(f"ERROR 6B: {e}")
                out.append(SEP)

                # ── 6C: POST getsms/number untuk range pertama ──
                out.append("[ 6C ] POST getsms/number — numbers fragment (raw, range pertama)")
                out.append(SEP2)
                try:
                    csrf6c = _get_csrf_cached(sc, RECV_URL) or sess.get("recv_csrf","")
                    # Coba ambil range dari hasil 6B
                    ranges6c = []
                    try:
                        for m in re.finditer(r"toggleRange\s*\(['\"]([^'\"]+)['\"],\s*['\"]([^'\"]+)['\"]", h6b):
                            ranges6c.append({"name": m.group(1), "id": m.group(2)})
                    except Exception:
                        pass

                    if not ranges6c:
                        out.append("  Tidak ada range dari 6B, skip 6C")
                    else:
                        rng6c = ranges6c[0]
                        out.append(f"  Range target: {rng6c['name']} (id={rng6c['id']})")
                        post6c = {
                            "_token": csrf6c,
                            "start":  today_s,
                            "end":    today_s,
                        ...
                        r6c = sc.post(f"{BASE_URL}/portal/sms/received/getsms/number",
                                      data=post6c,
                                      headers={
                                          "Accept":           "text/html, */*; q=0.01",
                                          "X-Requested-With": "XMLHttpRequest",
                        ...
                        out.append(f"  Status  : {r6c.status_code}")
                        out.append(f"  Size    : {len(h6c)} chars")
                        out.append("")
                        out.append("[ FULL RAW RESPONSE ]")
                        out.append(h6c)
                        out.append("")
                        out.append("[ toggleNum*() calls found ]")
                        for m in re.finditer(r"toggleNum\w*\s*\(['\"]?(\d{7,15})['\"]?\s*,\s*['\"]?([^'\")\s]+)['\"]?", h6c):
                            out.append(f"  number={m.group(1)} id={m.group(2)}")

                except Exception as e:
                    out.append(f"ERROR 6C: {e}")
                out.append(SEP)

                # ── 6D: POST getsms/number/sms untuk nomor pertama ──
                out.append("[ 6D ] POST getsms/number/sms — pesan fragment (raw, nomor pertama)")
                out.append(SEP2)
                try:
                    # Coba ambil nomor dari hasil 6C
                    nums6d = []
                    try:
                        for m in re.finditer(r"toggleNum\w*\s*\(['\"]?(\d{7,15})['\"]?\s*,\s*['\"]?([^'\")\s]+)['\"]?", h6c):
                            nums6d.append({"number": m.group(1), "id": m.group(2)})
                    except Exception:
                        pass

                    if not nums6d or not ranges6c:
                        out.append("  Tidak ada nomor dari 6C, skip 6D")
                    else:
                        num6d = nums6d[0]
                        rng6d = ranges6c[0]
                        csrf6d = _get_csrf_cached(sc, RECV_URL) or sess.get("recv_csrf","")
                        ...
                        r6d = sc.post(f"{BASE_URL}/portal/sms/received/getsms/number/sms",
                                      data=post6d,
                                      headers={
                                          "Accept":           "text/html, */*; q=0.01",
                                          "X-Requested-With": "XMLHttpRequest",
                        ...
                        out.append(f"  Status  : {r6d.status_code}")
                        out.append(f"  Size    : {len(h6d)} chars")
                        out.append("")
                        out.append("[ FULL RAW RESPONSE ]")
                        out.append(h6d)
                        out.append("")
                        out.append("[ div.msg-text elements ]")
                        soup6d = BeautifulSoup(h6d, "html.parser")
                        for el in soup6d.select("div.msg-text, td.msg-text, p.msg-text"):
                            out.append(f"  → {el.get_text(strip=True)[:200]}")
                except Exception as e:
                    out.append(f"ERROR 6D: {e}")
                out.append(SEP)

        except Exception as e:
            out.append(f"ERROR STEP 6: {e}")
        out.append(SEP)
...
# ════════════════════════════════════════════════════════
# /debug/raw — Dump RAW response dari URL iVAS apapun
# Usage: /debug/raw?url=https://www.ivasms.com/...&method=GET|POST
#        &account=email@gmail.com&from=2026-03-13&to=2026-03-13
# ════════════════════════════════════════════════════════
@app.route("/debug/raw")
def debug_raw():
    """
    Dump FULL raw HTML/JSON dari URL iVAS apapun via session akun.
    Berguna untuk reverse-engineer endpoint baru.

    Params:
      url     = full URL iVAS (default: RECV_URL)
      method  = GET | POST (default: GET)
      account = email akun (default: akun pertama)
      from    = tanggal from YYYY-MM-DD (untuk POST)
      to      = tanggal to   YYYY-MM-DD (untuk POST)
      range   = nama range   (untuk POST getsms/number)
      number  = nomor HP     (untuk POST getsms/number/sms)
      csrf_src= URL halaman sumber CSRF (default: RECV_URL)
    """
    target_url  = request.args.get("url",     RECV_URL).strip()
    ...
    csrf_src    = request.args.get("csrf_src", RECV_URL).strip()

    target_acc  = next((a for a in ACCOUNTS if a["email"] == acc_email), ACCOUNTS[0])
    SEP  = "=" * 70
    SEP2 = "-" * 50
    out  = []
    out.append("[DEBUG RAW] Dump raw response iVAS")
    ...
    try:
        ...
        sc   = sess["scraper"]
        csrf = _get_csrf_cached(sc, csrf_src) or sess.get("recv_csrf","") or sess.get("csrf","")

        out.append(f"CSRF    : {csrf[:50] if csrf else 'MISSING'}")
        out.append(f"CSRF src: {csrf_src}")
        out.append(SEP)

        if method == "GET":
            r = sc.get(target_url, timeout=15)
        else:
            post_data = {"_token": csrf, "from": from_date, "to": to_date}
            if range_name:
                post_data["range"]  = range_name
                post_data["start"]  = from_date
                post_data["end"]    = to_date
                post_data.pop("from", None)
                post_data.pop("to",   None)
            if phone_num:
                post_data["Number"] = phone_num
                post_data["Range"]  = range_name
            post_hdrs = {
                "Accept":           "text/html, */*; q=0.01",
                "X-Requested-With": "XMLHttpRequest",
            ...
            out.append(SEP2)
            r = sc.post(target_url, data=post_data, headers=post_hdrs, timeout=15)
    ...
    out.append("")
    out.append("Usage: /debug/raw?url=URL&method=GET|POST&account=EMAIL&from=YYYY-MM-DD&to=YYYY-MM-DD")
    out.append("       &range=RANGE_NAME&number=PHONE&csrf_src=URL_SUMBER_CSRF")
    return Response("\n".join(out), mimetype="text/plain; charset=utf-8")


# ════════════════════════════════════════════════════════
# /debug/getsms-raw — Khusus test POST getsms chain lengkap
# ════════════════════════════════════════════════════════
@app.route("/debug/getsms-raw")
def debug_getsms_raw():
    """
    Test full chain: ranges → numbers → messages, dengan dump RAW di setiap step.
    ...
    out.append(f"Account : {target_acc['email']}")
    out.append(f"Range   : {from_date} → {to_date}")
    out.append(SEP)
    ...
    try:
        ...
        def _csrf():
            return _get_csrf_cached(sc, RECV_URL) or sess.get("recv_csrf","")
        ...
        def _post(path, data):
            ...
            return sc.post(f"{BASE_URL}{path}", data=data, headers=hdrs, timeout=15)

        # ── CHAIN 1: Ranges ──
        out.append("CHAIN 1: POST /portal/sms/received/getsms")
        out.append(SEP2)
        r1 = _post("/portal/sms/received/getsms", {"from": from_date, "to": to_date})
        ...
        # Parse ranges
        ranges = []
        for m in re.finditer(r"toggleRange\s*\(['\"]([^'\"]+)['\"],\s*['\"]([^'\"]+)['\"]", h1):
            ranges.append({"name": m.group(1), "id": m.group(2)})
        out.append(f"[ Ranges parsed: {len(ranges)} ]")
        for rng in ranges:
            out.append(f"  {rng['name']} → {rng['id']}")
        out.append(SEP)

        if not ranges:
            out.append("0 ranges — stop. Tidak ada SMS di tanggal ini.")
            return Response("\n".join(out), mimetype="text/plain; charset=utf-8")

        # ── CHAIN 2: Numbers (semua range) ──
        out.append("CHAIN 2: POST /portal/sms/received/getsms/number (semua range)")
        out.append(SEP2)
        all_tasks = []
        for rng in ranges:
            out.append(f"  Range: {rng['name']}")
            r2 = _post("/portal/sms/received/getsms/number", {
                "start": from_date, "end": to_date, "range": rng["name"]
            })
            h2 = decode_response(r2)
            out.append(f"  Status: {r2.status_code}  Size: {len(h2)} chars")
            out.append(f"  [ RAW ]")
            out.append(f"  {h2[:1500]}")
            out.append("")
            nums = []
            for m in re.finditer(r"toggleNum\w*\s*\(['\"]?(\d{{7,15}})['\"]?\s*,\s*['\"]?([^'\")\s]+)['\"]?", h2):
                nums.append({"number": m.group(1), "id": m.group(2)})
            out.append(f"  Parsed numbers: {[n['number'] for n in nums]}")
            for n in nums:
                all_tasks.append({"number": n["number"], "num_id": n["id"], "range": rng["name"]})
            out.append(SEP2)
        out.append(SEP)

        if not all_tasks:
            out.append("0 nomor di semua range — stop.")
            return Response("\n".join(out), mimetype="text/plain; charset=utf-8")

        # ── CHAIN 3: SMS (semua nomor, max 5 agar tidak timeout) ──
        out.append("CHAIN 3: POST /portal/sms/received/getsms/number/sms (max 5 nomor)")
        out.append(SEP2)
        for task in all_tasks[:5]:
            out.append(f"  Number: {task['number']}  Range: {task['range']}")
            r3 = _post("/portal/sms/received/getsms/number/sms", {
                "start":  from_date,
                "end":    to_date,
                "Number": task["number"],
                "Range":  task["range"],
            ...
            out.append(f"  div.msg-text found: {msgs3}")
            out.append(SEP2)
    ...
    out.append("")
    out.append("Usage: /debug/getsms-raw?account=EMAIL&from=YYYY-MM-DD&to=YYYY-MM-DD")
    return Response("\n".join(out), mimetype="text/plain; charset=utf-8")
