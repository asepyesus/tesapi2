...
# ── CapSolver API Key untuk bypass Cloudflare Turnstile ──
...
def _solve_turnstile(sitekey: str, page_url: str) -> str:
    """
    Auto-solve Cloudflare Turnstile via CapSolver API.
    Docs: https://docs.capsolver.com/en/guide/antibots/cloudflare_turnstile/
    """
    if not _CAPSOLVER_KEY:
        logger.warning("[TURNSTILE] Tidak ada CAPSOLVER_KEY")
        return ""
    try:
        logger.info(f"[TURNSTILE] Creating task sitekey={sitekey[:30]}...")
        r1 = requests.post(
        ...
        data1   = r1.json()
        task_id = data1.get("taskId", "")
        if not task_id:
            logger.error(f"[TURNSTILE] Create task gagal: {data1}")
            return ""

        logger.info(f"[TURNSTILE] Task created id={task_id}, polling...")
        ...
        for attempt in range(30):
            ...
            if status == "ready":
                token = data2.get("solution", {}).get("token", "")
                if token:
                    logger.info(f"[TURNSTILE] SOLVED! token={token[:40]}... (attempt {attempt+1})")
                    return token
                logger.error(f"[TURNSTILE] Status ready tapi token kosong: {data2}")
                return ""

            if status == "processing":
                logger.debug(f"[TURNSTILE] Still processing... attempt={attempt+1}")
                continue

            # Error
            err_code = data2.get("errorCode", "")
            err_desc = data2.get("errorDescription", "")
            logger.error(f"[TURNSTILE] Error: {err_code} — {err_desc}")
            return ""
...
def login_account(account):
    ...
    try:
        ...
        if not sitekey:
            sk_m = re.search(r'data-sitekey=["\x27]([A-Za-z0-9_-]{10,})["\x27]', page_html)
            sitekey = sk_m.group(1) if sk_m else ""

        logger.info(f"[LOGIN] sitekey={'FOUND' if sitekey else 'NOT FOUND'} {email}")

        # ── Solve Turnstile ──
        turnstile_token = ""
        if sitekey:
            turnstile_token = _solve_turnstile(sitekey, LOGIN_URL)
            if not turnstile_token:
                logger.warning(f"[LOGIN] Turnstile solve gagal untuk {email}, coba tanpa token...")
        ...
        if "/login" in getattr(resp, "url", ""):
            ...
            if "turnstile" in resp_html.lower() and not turnstile_token:
                return {"ok": False,
                        "error": "Turnstile required — solver gagal, cek CAPSOLVER_KEY",
                        "email": email}
