...
def _scrape_csrf(scraper, page_url):
    """
    Scrape CSRF token dari halaman — dipanggil oleh _get_csrf_cached.
    Prioritas: meta csrf-token → input _token → JS inline.
...
def get_ranges(account, from_date, to_date):
    ...
    def _parse_ranges(html):
        """
        Parse daftar range dari HTML fragment iVAS.

        Format CONFIRMED dari debug step6:
          <div class="rng" onclick="toggleRange('TOGO 1613','TOGO_1613')">
            <span class="rname">TOGO 1613</span>
          </div>
        "No SMS found" = tidak ada SMS di tanggal itu, bukan bug.
        """
        # Pass 1: single-quote onclick — FORMAT UTAMA iVAS
        ...
        # Pass 3: BeautifulSoup — parse onclick attr dari div.rng
        if not result:
            soup = BeautifulSoup(html, "html.parser")
            for div in soup.select("div.rng, [onclick*=toggleRange]"):
                oc = div.get("onclick", "")
                m2 = re.search(r"toggleRange\s*\(\s*'([^']+)'\s*,\s*'([^']+)'", oc)
                if not m2:
                    m2 = re.search(r'toggleRange\s*\(\s*"([^"]+)"\s*,\s*"([^"]+)"', oc)
                if m2:
                    ...
                    if name:
                        ...
                        if not sub:
                            ...
                            if sib and hasattr(sib, "get"):
                                sub = sib if sib.get("id","").startswith("sp_") else (sib.select_one("[id^='sp_']") if hasattr(sib,"select_one") else None)
                        rid = sub["id"].replace("sp_", "") if sub else name.replace(" ", "_").upper()
                        _add(name, rid)

        # Pass 4: id="sp_XXXX" fallback
        if not result:
            ...
            for div in soup2.select("[id^='sp_']"):
                ...
                if prev and hasattr(prev, "select_one"):
                    rname_el = prev.select_one("span.rname")
    ...
    if step_filter in ("all", "6"):
        ...
        try:
            ...
            if not sess.get("ok"):
                ...
                # ── 6A: shell page + SPA/Socket analysis ──
                out.append("[ 6A ] GET /portal/sms/received — shell + SPA/Socket")
                out.append(SEP2)
                try:
                    r6a = sc_s.get(RECV_URL, timeout=15)
                    h6a = decode_response(r6a)
                    out.append(f"Status : {r6a.status_code}  Size: {len(h6a)} chars  URL: {r6a.url}")
                    out.append("")

                    out.append("[ Socket.IO io() endpoints ]")
                    for ln in h6a.split("\n"):
                        ls = ln.strip()
                        if "io(" in ls and ("ivasms" in ls or "socket" in ls.lower()):
                            out.append(f"  {ls[:150]}")
                    ...
                    out.append("")
                    out.append("[ GetSMS() function ]")
                    idx_gs = h6a.find("function GetSMS")
                    if idx_gs >= 0:
                        out.append(h6a[idx_gs:idx_gs+1200])
                    else:
                        out.append("  (tidak ada di halaman ini)")

                    has_spa    = "SPA.navigate" in h6a or "SPA.render" in h6a
                    has_socket = "socket.io" in h6a.lower() or "ivasms.com:2087" in h6a
                    out.append("")
                    out.append(f"[ SPA mode  ] {'YES' if has_spa else 'NO'}")
                    out.append(f"[ Socket.IO ] {'YES port 2087' if has_socket else 'NO'}")

                    out.append("")
                    out.append("[ _token dari form ]")
                    soup6a = BeautifulSoup(h6a, "html.parser")
                    for inp in soup6a.find_all("input", {"name": "_token"}):
                        out.append(f"  {inp.get('value','')[:80]}")

                    out.append("")
                    out.append("[ External <script src> ]")
                    for sc_tag in soup6a.find_all("script", src=True):
                        out.append(f"  {sc_tag['src'][:120]}")

                except Exception as e6a:
                    out.append(f"ERROR 6A: {e6a}")
                out.append(SEP)

                # ── 6B: POST getsms (ranges) ──
                out.append("[ 6B ] POST /portal/sms/received/getsms — ranges (full raw)")
                out.append(SEP2)
                h6b      = ""
                ranges6b = []
                try:
                    csrf6b = _get_csrf_cached(sc_s, RECV_URL) or sess.get("recv_csrf","")
                    post6b = {"_token": csrf6b, "from": today_s, "to": today_s}
                    hdrs6b = {
                        "Accept":           "text/html, */*; q=0.01",
                        "X-Requested-With": "XMLHttpRequest",
                        "Content-Type":     "application/x-www-form-urlencoded; charset=UTF-8",
                    ...
                    r6b = sc_s.post(f"{BASE_URL}/portal/sms/received/getsms",
                                    data=post6b, headers=hdrs6b, timeout=15)
                    h6b = decode_response(r6b)
                    out.append(f"Status : {r6b.status_code}  Size: {len(h6b)} chars")
                    out.append(f"CSRF   : {csrf6b[:60] if csrf6b else 'MISSING'}")
                    out.append("")
                    out.append("[ FULL RAW RESPONSE ]")
                    out.append(h6b)
                    out.append("")

                    out.append("[ toggleRange() parsed ]")
                    for m in re.finditer(r"toggleRange\s*\(\s*'([^']+)'\s*,\s*'([^']+)'\s*\)", h6b):
                        ranges6b.append({"name": m.group(1), "id": m.group(2)})
                        out.append(f"  name={m.group(1)}  id={m.group(2)}")
                    for m in re.finditer(r'toggleRange\s*\(\s*"([^"]+)"\s*,\s*"([^"]+)"\s*\)', h6b):
                        if not any(r["name"] == m.group(1) for r in ranges6b):
                            ranges6b.append({"name": m.group(1), "id": m.group(2)})
                            out.append(f"  name={m.group(1)}  id={m.group(2)}")
                    if not ranges6b:
                        soup6b = BeautifulSoup(h6b, "html.parser")
                        for div in soup6b.select("div.rng, [onclick*=toggleRange]"):
                            oc = div.get("onclick","")
                            m2 = re.search(r"toggleRange[^(]*[(]([^,)]+)", oc)
                            if m2:
                                name2 = m2.group(1).strip("'\" ")
                                ranges6b.append({"name": name2, "id": name2.replace(" ","_").upper()})
                                out.append(f"  [BS] name={name2}")
                        if not ranges6b:
                            out.append("  (0 range — tidak ada SMS hari ini)")
                            soup6bx = BeautifulSoup(h6b, "html.parser")
                            divs = soup6bx.find_all("div", class_=True)[:5]
                            out.append(f"  Div classes: {[d.get('class') for d in divs]}")

                except Exception as e6b:
                    out.append(f"ERROR 6B: {e6b}")
                out.append(SEP)

                # ── 6C: POST getsms/number ──
                out.append("[ 6C ] POST getsms/number — numbers (full raw)")
                out.append(SEP2)
                h6c    = ""
                nums6c = []
                try:
                    if not ranges6b:
                        out.append("  0 ranges dari 6B — skip 6C")
                    else:
                        rng6c  = ranges6b[0]
                        csrf6c = _get_csrf_cached(sc_s, RECV_URL) or sess.get("recv_csrf","")
                        out.append(f"  Range: {rng6c['name']} (id={rng6c['id']})")
                        post6c = {"_token": csrf6c, "start": today_s, "end": today_s, "range": rng6c["name"]}
                        r6c = sc_s.post(f"{BASE_URL}/portal/sms/received/getsms/number",
                                        data=post6c,
                                        headers={
                                            "Accept":           "text/html, */*; q=0.01",
                                            "X-Requested-With": "XMLHttpRequest",
                        ...
                        out.append(h6c)
                        out.append("")
                        out.append("[ toggleNum*() parsed ]")
                        for m in re.finditer(r"toggleNum\w*\s*\(\s*'(\d{7,15})'\s*,\s*'([^']+)'\s*\)", h6c):
                            nums6c.append({"number": m.group(1), "id": m.group(2)})
                            out.append(f"  number={m.group(1)}  id={m.group(2)}")
                        if not nums6c:
                            out.append("  (0 nomor)")
                except Exception as e6c:
                    out.append(f"ERROR 6C: {e6c}")
                out.append(SEP)

                # ── 6D: POST getsms/number/sms ──
                out.append("[ 6D ] POST getsms/number/sms — messages (full raw)")
                out.append(SEP2)
                try:
                    if not nums6c or not ranges6b:
                        ...
                        num6d  = nums6c[0]
                        rng6d  = ranges6b[0]
                        csrf6d = _get_csrf_cached(sc_s, RECV_URL) or sess.get("recv_csrf","")
                        ...
                        r6d = sc_s.post(f"{BASE_URL}/portal/sms/received/getsms/number/sms",
                                        data=post6d,
                                        headers={
                                            "Accept":           "text/html, */*; q=0.01",
                                            "X-Requested-With": "XMLHttpRequest",
                ...
                except Exception as e6d:
                    out.append(f"ERROR 6D: {e6d}")
                out.append(SEP)
...
def debug_raw_v2():
    """
    Dump FULL raw HTML dari URL iVAS apapun via session akun.
    Usage: /debug/raw?url=https://www.ivasms.com/...&method=GET|POST
           &account=EMAIL&from=YYYY-MM-DD&to=YYYY-MM-DD&range=NAME&number=PHONE
    """
    ...
    if not target_url or not target_url.startswith("http"):
        return Response(
            "ERROR: parameter url= harus diisi URL lengkap.\n"
            "Contoh: /debug/raw?url=https://www.ivasms.com/portal/sms/received/getsms"
            "&method=POST&account=kicenofficial@gmail.com&from=2026-03-13&to=2026-03-13",
            mimetype="text/plain"
        )
    ...
    try:
        ...
        if method == "GET":
            ...
            if range_name:
                post_data.update({"start": from_date, "end": to_date, "range": range_name})
            else:
                post_data.update({"from": from_date, "to": to_date})
            if phone_num:
    ...
    out.append("")
    out.append("Usage: /debug/raw?url=URL&method=GET|POST&account=EMAIL&from=YYYY-MM-DD&to=YYYY-MM-DD&range=NAME&number=PHONE")
    return Response("\n".join(out), mimetype="text/plain; charset=utf-8")
...
def debug_getsms_raw_v2():
    """
    Test full chain ranges->numbers->messages dengan full raw dump.
    ...
    SEP2 = "-" * 50
    out  = []
    out.append("[DEBUG GETSMS-RAW] Full chain")
    out.append(f"Account : {target_acc['email']}")
    ...
    try:
        ...
        def _post_g(path, data):
            data["_token"] = _csrf_g()
            return sc_g.post(f"{BASE_URL}{path}", data=data,
                             headers={
                                 "Accept":           "text/html, */*; q=0.01",
                                 "X-Requested-With": "XMLHttpRequest",
                                 "Content-Type":     "application/x-www-form-urlencoded; charset=UTF-8",
        ...
        out.append(SEP2)
        r1 = _post_g("/portal/sms/received/getsms", {"from": from_date, "to": to_date})
        ...
        for m in re.finditer(r'toggleRange\s*\(\s*"([^"]+)"\s*,\s*"([^"]+)"\s*\)', h1):
            if not any(r["name"] == m.group(1) for r in ranges):
                ranges.append({"name": m.group(1), "id": m.group(2)})
        ...
        if not ranges:
            out.append("0 ranges — tidak ada SMS di tanggal ini.")
            return Response("\n".join(out), mimetype="text/plain; charset=utf-8")

        # CHAIN 2
        out.append("CHAIN 2: POST getsms/number")
        out.append(SEP2)
        all_tasks = []
        for rng in ranges:
            out.append(f"Range: {rng['name']}")
            r2 = _post_g("/portal/sms/received/getsms/number",
        ...
        for task in all_tasks[:5]:
            ...
            r3 = _post_g("/portal/sms/received/getsms/number/sms",
