                ...
                if resp and resp.status_code == 200:
                    ...
                    for row in rows:
                        if isinstance(row, dict):
                            ...
                            msg = html_lib.unescape(re.sub(r'<[^>]+>', '', str(row.get("messagedata", "")))).strip()
                            rcv = str(row.get("senttime", ""))
                            rng = html_lib.unescape(re.sub(r'<[^>]+>', '', str(row.get("range", "")))).strip()
                        elif isinstance(row, list) and len(row) >= 4:
                            raw_sid = str(row[2])
                            raw_num = str(row[1])
                            msg     = html_lib.unescape(re.sub(r'<[^>]+>', '', str(row[3]))).strip()
                            rcv     = str(row[4]) if len(row) > 4 else ""
                            rng     = html_lib.unescape(re.sub(r'<[^>]+>', '', str(row[0]))).strip()
                        else:
                            continue
        ...
        if resp and resp.status_code == 200:
            ...
            try:
                ...
                for row in rows:
                    ...
                    # Clean message: decode entities + unicode escapes + strip HTML
                    def _clean_msg(raw: str) -> str:
                        s = html_lib.unescape(raw)
                        s = re.sub(r'<script[\s\S]*?</script>', '', s, flags=re.IGNORECASE)
                        s = re.sub(r'<[^>]+>', '', s)
                        # Normalize newlines dan whitespace berlebih
                        s = re.sub(r'\r\n|\r', '\n', s)
                        s = re.sub(r'[ \t]+', ' ', s)
                        s = s.strip()
                        return s
                    ...
                    msg = _clean_msg(msg)
                    rng = html_lib.unescape(re.sub(r'<[^>]+>', '', rng)).strip()
