...
def get_ranges(account, from_date, to_date):
    ...
    # ── Attempt 1: POST dengan iVAS date format (M/D/YYYY) — CONFIRMED ────────
    resp1, _ = do_request(
        account, "POST",
    ...
    if resp1 and resp1.status_code == 200:
        ...
        _parse_ranges(html1)
        logger.info(f"[RANGES] POST (iVAS fmt) → {len(result)} ranges, html={len(html1)}c")

    # ── Attempt 2: POST dengan date format DD/MM/YYYY langsung ────────────────
    if not result:
        resp2, _ = do_request(
            account, "POST",
            f"{BASE_URL}/portal/sms/received/getsms",
        ...
        if resp2 and resp2.status_code == 200:
            ...
            _parse_ranges(html2)
            logger.info(f"[RANGES] POST (raw fmt) → {len(result)} ranges, html={len(html2)}c")

    # ── Attempt 3: POST dengan format YYYY-MM-DD ──────────────────────────────
    if not result:
        try:
            d      = datetime.strptime(from_date, "%d/%m/%Y")
            iso_dt = d.strftime("%Y-%m-%d")
            resp3, _ = do_request(
                account, "POST",
                f"{BASE_URL}/portal/sms/received/getsms",
                data={"from": iso_dt, "to": iso_dt},
                headers=ajax_hdrs(),
            )
            if resp3 and resp3.status_code == 200:
                html3 = decode_response(resp3)
                _parse_ranges(html3)
                logger.info(f"[RANGES] POST (ISO fmt) → {len(result)} ranges, html={len(html3)}c")
        except Exception:
            pass
...
def debug_sms_deep():
    ...
    attempts_step1 = [
        ("Attempt 1 — POST from/to iVAS format M/D/YYYY (UTAMA)",
         {"from": ivas_date, "to": ivas_date_to}),
        ("Attempt 2 — POST from/to DD/MM/YYYY langsung",
         {"from": date_str, "to": date_str}),
        ("Attempt 3 — POST from/to YYYY-MM-DD",
         {"from": datetime.strptime(date_str, "%d/%m/%Y").strftime("%Y-%m-%d"),
          "to":   datetime.strptime(date_str, "%d/%m/%Y").strftime("%Y-%m-%d")}),
    ]
