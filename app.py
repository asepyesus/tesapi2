...
def _scrape_csrf(scraper, page_url):
    """
    Scrape CSRF token langsung dari halaman page_url.
...
def debug_login_flow():
    ...
    # ── STEP 6: FULL RAW HTML DUMP + SPA/Socket.IO analysis ──
    if step_filter in ("all", "6"):
        ...
        try:
            ...
            if not sess.get("ok"):
                ...
                # ── 6A: GET shell page + SPA/Socket analysis ──
                out.append("[ 6A ] GET /portal/sms/received — shell + SPA/Socket analysis")
                out.append(SEP2)
                try:
                    ...
                    for m in re.finditer(r"io\(['\"]([^'\"]+)['\"]", h6a):
                        out.append(f"  -> {m.group(1)}")

                    # socket.on events
                    out.append("")
                    out.append("[ socket.on() events ]")
                    for m in re.finditer(r"socket\.on\(['\"]([^'\"]+)['\"]", h6a):
                        out.append(f"  -> {m.group(1)}")

                    # SPA/AJAX URLs
                    # SPA/AJAX URLs
                    out.append("")
                    out.append("[ SPA $.ajax / portal URLs ]")
                    _spa_hits = [l.strip() for l in h6a.split(chr(10)) if "portal" in l and ("url" in l.lower() or "ajax" in l.lower() or "getsms" in l.lower())]
                    for _hit in _spa_hits[:20]: out.append(f"  {_hit[:150]}")
                    if not _spa_hits: out.append("  (tidak ditemukan)")
                    ...
                    if idx_gs >= 0:
                        ...
                        out.append("  (GetSMS tidak ditemukan di halaman ini)")

                    # SPA / Socket flags
                    has_spa    = "SPA.navigate" in h6a or "SPA.render" in h6a
                    has_socket = "socket.io" in h6a.lower() or "ivasms.com:2087" in h6a
                    out.append("")
                    out.append(f"[ SPA mode    ] {'YES - data load via GET $.ajax' if has_spa else 'NO'}")
                    out.append(f"[ Socket.IO   ] {'YES - port 2087' if has_socket else 'NO'}")

                    # CSRF dari JS
                    out.append("")
                    out.append("[ _token di inline JS ]")
                    out.append("[ _token di inline JS ]")
                    _tok_hits = [l.strip() for l in h6a.split(chr(10)) if "_token" in l and any(c.isalpha() for c in l[l.find("_token")+7:l.find("_token")+80])]
                    for _th in _tok_hits[:10]: out.append(f"  {_th[:160]}")
                    out.append("")
                ...
                # ── 6B: POST getsms (ranges) — full raw ──
                ...
                try:
                    ...
                    out.append(f"CSRF   : {csrf6b[:50] if csrf6b else 'MISSING'}")
                    out.append("")
                    out.append("[ FULL RAW RESPONSE ]")
                    out.append(h6b)
                    out.append("")
                    out.append("[ toggleRange() parsed ]")
                    for m in re.finditer(r"toggleRange.{0,5}\([^)]+\)", h6b):
                        ranges6b.append({"name": m.group(1), "id": m.group(2)})
                        out.append(f"  name={m.group(1)}  id={m.group(2)}")
                    if not ranges6b:
                        out.append("  (0 range — coba GET method di 6B.2)")

                    # 6B.2: Coba GET sebagai fallback (SPA-style)
                    out.append("")
                    out.append("[ 6B.2 ] GET /portal/sms/received/getsms (SPA fallback)")
                    r6b2 = sc_s.get(f"{BASE_URL}/portal/sms/received/getsms",
                                    params={"from": today_s, "to": today_s},
                                    headers={"X-Requested-With": "XMLHttpRequest",
                                             "Referer": RECV_URL},
                                    timeout=15)
                    h6b2 = decode_response(r6b2)
                    out.append(f"Status : {r6b2.status_code}  Size: {len(h6b2)} chars")
                    out.append(h6b2[:1000])
                    for m in re.finditer(r"toggleRange.{0,5}\([^)]+\)", h6b):
                        if not any(r["name"] == m.group(1) for r in ranges6b):
                            ranges6b.append({"name": m.group(1), "id": m.group(2)})
                            out.append(f"  [GET] name={m.group(1)}  id={m.group(2)}")

                except Exception as e:
                    out.append(f"ERROR 6B: {e}")
                out.append(SEP)

                # ── 6C: POST getsms/number (numbers) ──
                ...
                try:
                    if not ranges6b:
                        ...
                        for m in re.finditer(r"toggleNum\w*\(\d{7,15}", h6c):
                ...
                try:
                    if not nums6c or not ranges6b:
                        ...
                        out.append(f"[ div.msg-text found ] {msgs6d}")
                except Exception as e:
...
def debug_raw_new():
    """
    Dump FULL raw HTML dari URL iVAS apapun.
    Usage: /debug/raw?url=URL&method=GET|POST&account=EMAIL&from=YYYY-MM-DD&to=YYYY-MM-DD
           &range=RANGE_NAME&number=PHONE
    """
    ...
    try:
        ...
        if method == "GET":
            ...
            if range_name:
                ...
                post_data.pop("from", None); post_data.pop("to", None)
            if phone_num:
...
def debug_getsms_raw_new():
    ...
    try:
        ...
        ranges = []
        for m in re.finditer(r"toggleRange[^(]*\(([^,]+),\s*([^)]+)\)", h1):
        ...
        if not ranges:
            out.append("0 ranges — selesai.")
            return Response("\n".join(out), mimetype="text/plain; charset=utf-8")

        # CHAIN 2: Numbers
        out.append("CHAIN 2: POST getsms/number (semua range)")
        out.append(SEP2)
        all_tasks = []
        for rng in ranges:
            ...
            out.append(h2[:1000])
            nums = []
            for m in re.finditer(r"toggleNum\w*\((\d{7,15})[^,]*,([^)]+)\)", h2):
        ...
        if not all_tasks:
            out.append("0 nomor — selesai.")
            return Response("\n".join(out), mimetype="text/plain; charset=utf-8")

        # CHAIN 3: SMS (max 5)
        out.append("CHAIN 3: POST getsms/number/sms (max 5 nomor)")
        out.append(SEP2)
