...
def debug_cf_login():
    ...
    try:
        ...
        out.append(f"CF ray       : {r1.headers.get('cf-ray', 'none')}")
        ...
        ts_token = ""
        if sitekey:
            out.append(f"Solving Turnstile via CapSolver (key={_CAPSOLVER_KEY[:20]}...)...")
            ts_token = _solve_turnstile(sitekey, LOGIN_URL)
            out.append(f"Turnstile token: {'SOLVED: '+ts_token[:40]+'...' if ts_token else 'FAILED!'}")
        else:
            out.append("Sitekey tidak ditemukan — skip solve")
        out.append("")
        ...
        out.append(f"Login OK     : {'YES ✅' if '/portal' in r2.url else 'NO ❌'}")
        out.append(f"HTML snippet : {h2[:300]}")
        out.append(SEP)
    except Exception as e:
        import traceback
        out.append(f"ERROR: {e}")
