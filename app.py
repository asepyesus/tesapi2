    ...
    try:
        ...
        for _attempt, _dummy in enumerate(_dummy_tokens):
            ...
            if _attempt < len(_dummy_tokens) - 1:
                ...
                try:
                    _rp = scraper.get(LOGIN_URL, timeout=20)
...
_PRESET_COOKIES = [
    {
        ...
        "cookies": {
            "XSRF-TOKEN":      "%3D",
            "ivas_sms_session": "%3D",
        }
    },
    {
        "email": "ceptampan58@gmail.com",
        "cookies": {
            "XSRF-TOKEN":      "ey.Jpdil6lk9hcmEyV1ZGbDJscjVpV3A1V3M0SkE9PSIsInZhbHVlijoiUk5EMytla2d0a1dHOV.JWSHI1SXF4dHhvZkM4UnB2Q3ZobGc2Q3dUc09uV2FFU2QwVTBxS2.JWYnViTm9GNGNDSDFRYXlmb1F5ckE5SXFaMitEVUZKMzRpNkVCd2dUMHEzcWE5UXZqS3hBQ21hTjgwZHpEdnh0TGxXbTN2L0V6Q20ILCJtYWMIOIIzMjIjZJYOMJQYMmU2YWVIM2Q1NDgzNmE4MjM4Y2U20TRjZWY2OTFkZDQxYjg5N2NmZjhkMTNmYTFIOTIOZDRiliwidGFnljolino=",
            "ivas_sms_session": "ey.",
        }
    },
]
...
def _inject_preset_cookies():
    ...
    for entry in _PRESET_COOKIES:
        ...
        for name, value in cookies.items():
            ...
            scraper.cookies.set(name, str(value), domain="ivasms.com",     path="/")
        session_entry = {
            "ok":              True,
            "scraper":         scraper,
            "csrf":            "",
        ...
        logger.info(f"[PRESET] Cookies di-inject untuk {email} ({len(cookies)} cookies)")


def login_all_accounts(force=False):
    """
    Return list session aktif semua akun.
    Prioritas: cache dulu (termasuk cookie inject), baru login ulang kalau perlu.
    """
    ...
    with _session_lock:
        for acc in all_accounts:
            ...
            if cached and cached.get("ok"):
                ...
                logger.info(f"[SESSION] Cache HIT {acc['email']} via={cached.get('via','normal')}")
            else:
                need_login.append(acc)

    if sessions and not force:
        logger.info(f"[SESSION] {len(sessions)}/{len(all_accounts)} dari cache, skip login")
        return sessions

    if not need_login:
        return sessions

    # Fase 2: Login akun yang belum ada session
    logger.info(f"[SESSION] Login {len(need_login)} akun tanpa session...")
    with ThreadPoolExecutor(max_workers=max(len(need_login), 1)) as ex:
        futures = {ex.submit(login_account, acc): acc for acc in need_login}
        for future in as_completed(futures):
            result = future.result()
            with _session_lock:
                existing = _session_cache.get(result["email"])
                if existing and existing.get("ok") and existing.get("via") in ("cookie_inject", "preset_cookies"):
                    logger.info(f"[SESSION] Skip overwrite cookie session {result['email']}")
    ...
    logger.info(f"[SESSION] {len(sessions)}/{len(all_accounts)} akun aktif")
    return sessions
...
def debug_cf_login():
    ...
    try:
        ...
        # POST login
        login_data = {"email": target["email"], "password": target["password"], "_token": tok}

        r2 = sc.post(LOGIN_URL, data=login_data,
                     headers={"Content-Type": "application/x-www-form-urlencoded",
                              "Referer": LOGIN_URL, "Origin": BASE_URL},
        ...
        out.append(f"Login OK    : {'YES!' if '/portal' in r2.url or '/login' not in r2.url else 'NO'}")
        out.append("")
        out.append("HTML snippet:")
        out.append(h2[:500])
    except Exception as e:
        import traceback
        out.append(f"ERROR: {e}")
...
def set_cookies():
    ...
    if request.method == "GET" and not request.args.get("email"):
        guide = {
            ...
            "body": {
                ...
                "cookies": {
                    "laravel_session": "PASTE_DARI_BROWSER",
                    "XSRF-TOKEN":      "PASTE_DARI_BROWSER",
                    "remember_web_XXXX": "PASTE_JIKA_ADA (opsional)"
                },
                "verify": 1
            },
            "cara_ambil_cookies": [
                "1. Buka ivasms.com/login di browser",
                "2. Login manual (selesaikan Turnstile)",
                "3. DevTools (F12) → Application → Storage → Cookies → https://www.ivasms.com",
                "4. Copy nilai: laravel_session, XSRF-TOKEN, remember_web_* (jika ada)",
                "5. POST ke endpoint ini dengan body JSON di atas"
            ],
            "status_endpoint": "/cookies-status",
            "contoh_curl": (
                'curl -X POST /set-cookies '
                '-H "Content-Type: application/json" '
                '-d \'{"email":"akun@gmail.com","cookies":{"laravel_session":"VALUE","XSRF-TOKEN":"VALUE"}}\''
            )
        }
    ...
    # Build scraper baru dan inject cookies
    scraper = build_scraper()
    for name, value in cookies.items():
        scraper.cookies.set(name, str(value), domain="www.ivasms.com", path="/")
        scraper.cookies.set(name, str(value), domain="ivasms.com", path="/")
    ...
    # Kalau akun belum di ACCOUNTS, tambahkan supaya ikut di-loop
    if not _get_account(email):
        with _BOT_LOCK:
            _BOT_ACCOUNTS[email] = {
                "email":    email,
                "password": "",
