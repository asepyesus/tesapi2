...
def _scrape_csrf(scraper, page_url):
    """
    Scrape CSRF token FRESH langsung dari halaman iVAS.
    Dipanggil oleh _get_csrf_cached setiap kali cache miss/expired.

    Prioritas:
      1. <meta name="csrf-token" content="...">   ← paling reliable
      2. <input name="_token" value="...">
      3. JS inline: X-CSRF-TOKEN / _token / csrfToken
      4. window.Laravel.csrfToken (Laravel SPA pattern)
    ...
    try:
        ...
        if "/login" in r.url or r.status_code in (401, 403):
            logger.warning(f"[CSRF-SCRAPE] Session expired → {r.url}")
            return None

        if r.status_code != 200:
            logger.warning(f"[CSRF-SCRAPE] HTTP {r.status_code} dari {page_url}")
            return None

        html = decode_response(r)
        page_short = page_url.replace(BASE_URL, "")

        # ── Pass 1: <meta name="csrf-token"> ──
        soup = BeautifulSoup(html, "html.parser")
        meta = soup.find("meta", {"name": "csrf-token"})
        if meta and meta.get("content") and len(meta["content"]) > 10:
            tok = meta["content"]
            logger.info(f"[CSRF-SCRAPE] ✅ meta csrf-token dari {page_short}: {tok[:20]}...")
            return tok

        # ── Pass 2: <input name="_token"> ──
        inp = soup.find("input", {"name": "_token"})
        if inp and inp.get("value") and len(inp["value"]) > 10:
            tok = inp["value"]
            logger.info(f"[CSRF-SCRAPE] ✅ input _token dari {page_short}: {tok[:20]}...")
            return tok

        # ── Pass 3: JS inline patterns ──
        js_patterns = [
            r'["\']X-CSRF-TOKEN["\']\s*:\s*["\']([A-Za-z0-9_\-+/=]{20,})["\']',
            r'["\']_token["\']\s*[,:]?\s*["\']([A-Za-z0-9_\-+/=]{20,})["\']',
            r'csrfToken\s*=\s*["\']([A-Za-z0-9_\-+/=]{20,})["\']',
            r'csrf[_\-]?token\s*[=:]\s*["\']([A-Za-z0-9_\-+/=]{20,})["\']',
            r'window\.Laravel\s*=.*?csrfToken["\s]*:\s*["\']([A-Za-z0-9_\-+/=]{20,})["\']',
            r'meta\[["\']csrf[_\-]?token["\']\]\s*=\s*["\']([A-Za-z0-9_\-+/=]{20,})["\']',
        ]
        for pat in js_patterns:
            m = re.search(pat, html, re.IGNORECASE | re.DOTALL)
            if m:
                tok = m.group(1)
                logger.info(f"[CSRF-SCRAPE] ✅ JS inline dari {page_short}: {tok[:20]}...")
                return tok

        # ── Pass 4: cari semua string panjang yg mirip token (last resort) ──
        for m in re.finditer(r'["\']([A-Za-z0-9_\-+/=]{40,100})["\']', html):
            candidate = m.group(1)
            # Filter: token biasanya base64url, bukan URL atau path
            if "/" not in candidate and "." not in candidate:
                logger.info(f"[CSRF-SCRAPE] ✅ fallback token dari {page_short}: {candidate[:20]}...")
                return candidate

        logger.warning(f"[CSRF-SCRAPE] ❌ Token tidak ditemukan di {page_short} "
                       f"(HTTP {r.status_code}, size {len(html)}c) — mungkin halaman SPA/JS-rendered")
    except Exception as e:
        logger.warning(f"[CSRF-SCRAPE] ❌ Exception dari {page_url}: {e}")
    return None
...
def _get_csrf_cached(scraper, page_url):
    """
    CSRF dengan cache per-scraper per-page.
    - Cache TTL: 25 detik (iVAS token valid lebih lama, tapi kita refresh sering)
    - Cache miss / expired → panggil _scrape_csrf() fresh
    - Selalu log INFO supaya keliatan di Reviactyl console
    ...
    with _csrf_cache_lock:
        ...
        if cached:
            token, ts = cached
            age = now - ts
            if age < _CSRF_CACHE_TTL:
                logger.info(f"[CSRF] Cache HIT {page_short} (age {age:.0f}s): {token[:20]}...")
                return token
            # Expired
            logger.info(f"[CSRF] Cache EXPIRED {page_short} (age {age:.0f}s) — scrape fresh")
            del _csrf_cache[key]
        else:
            logger.info(f"[CSRF] Cache MISS {page_short} — scrape fresh")

    # Scrape fresh dari halaman
    token = _scrape_csrf(scraper, page_url)
    if token:
        with _csrf_cache_lock:
            _csrf_cache[key] = (token, now)
        logger.info(f"[CSRF] Cached fresh token {page_short}: {token[:20]}...")
    else:
        logger.warning(f"[CSRF] ❌ Scrape gagal {page_short} — akan fallback ke session CSRF")
    return token
...
def do_request(account, method, url, data=None, headers=None):
    ...
    for attempt in range(3):
        ...
        if method.upper() != "GET" and csrf_source_page:
            ...
            if fresh_csrf:
                csrf = fresh_csrf
                logger.info(f"[REQ] CSRF fresh OK untuk {url.replace(BASE_URL,'')}: {csrf[:20]}...")
            else:
                # Fallback ke session csrf
                csrf = session.get("recv_csrf") if "/portal/sms/received" in url else session["csrf"]
                logger.warning(f"[REQ] ⚠️  CSRF scrape gagal, fallback session csrf: {csrf[:20]}...")
        elif method.upper() != "GET":
            csrf = session.get("recv_csrf") if "/portal/sms/received" in url else session["csrf"]
            logger.info(f"[REQ] CSRF dari session (no map): {csrf[:20]}...")
        else:
            csrf = session["csrf"]
...
# ════════════════════════════════════════════════════════
# DEBUG CSRF — Test login + CSRF flow per akun
# ════════════════════════════════════════════════════════

@app.route("/debug/csrf")
def debug_csrf():
    """
    Debug FULL login + CSRF flow untuk setiap akun.

    Menampilkan step by step:
      STEP 1 : Login iVAS → csrf dari session
      STEP 2 : Scrape fresh CSRF dari setiap halaman portal
      STEP 3 : Test POST ke getsms pakai fresh CSRF vs session CSRF

    Usage:
      /debug/csrf
      /debug/csrf?account=email@gmail.com   ← fokus 1 akun
      /debug/csrf?nocache=1                 ← bypass cache, selalu fresh scrape

    Output: plain text, langsung bisa dibaca di browser/Reviactyl
    """
    import time as _t
    ...
    out.append(SEP)
    out.append(f"  DEBUG CSRF — {now}")
    out.append(f"  nocache={nocache}")
    out.append(SEP)
    out.append("")

    # Halaman portal yang kita test CSRF-nya
    test_pages = [
        f"{BASE_URL}/portal/sms/received",
        f"{BASE_URL}/portal/numbers/test",
        f"{BASE_URL}/portal/numbers",
        f"{BASE_URL}/portal/live/my_sms",
    ]

    sessions = login_all_accounts()
    if not sessions:
        out.append("❌ SEMUA AKUN GAGAL LOGIN")
        return Response("\n".join(out), mimetype="text/plain; charset=utf-8")

    for sess in sessions:
        email = sess["email"]
        if acc_filter and acc_filter not in email:
            continue
        ...
        # ── STEP 1: Cek CSRF dari session login ──
        out.append("")
        out.append("[ STEP 1 ] CSRF dari session login")
        out.append(f"  csrf      : {sess.get('csrf','(none)')[:60]}")
        out.append(f"  recv_csrf : {sess.get('recv_csrf','(none)')[:60]}")
        same = sess.get('csrf') == sess.get('recv_csrf')
        out.append(f"  Sama?     : {'YA (iVAS pakai 1 token global)' if same else 'BEDA (per-page token)'}")

        # ── STEP 2: Scrape fresh CSRF dari setiap halaman ──
        out.append("")
        out.append("[ STEP 2 ] Scrape fresh CSRF dari halaman portal")
        ...
        for page in test_pages:
            ...
            # Clear cache dulu kalau nocache=1
            if nocache:
                key = (id(scraper), page)
                with _csrf_cache_lock:
                    _csrf_cache.pop(key, None)

            token = _get_csrf_cached(scraper, page)
            elapsed = _t.time() - t0

            if token:
                same_as_session = token == sess.get("csrf") or token == sess.get("recv_csrf")
                out.append(f"  {page_short:<35} ✅ {token[:40]}...  ({elapsed:.2f}s)"
                           f"{'  ← SAMA dg session' if same_as_session else '  ← FRESH BEDA'}")
            else:
                out.append(f"  {page_short:<35} ❌ GAGAL ({elapsed:.2f}s)")

        # ── STEP 3: Test POST getsms pakai fresh CSRF ──
        out.append("")
        out.append("[ STEP 3 ] Test POST getsms pakai CSRF baru")
        account = next((a for a in _get_all_accounts() if a["email"] == email), None)
        if not account:
            out.append("  ⚠️  akun tidak ditemukan di pool")
            out.append("")
            continue

        today = datetime.now().strftime("%Y-%m-%d")

        # Test dengan fresh CSRF
        fresh_csrf = _scrape_csrf(scraper, f"{BASE_URL}/portal/sms/received")
        out.append(f"  Fresh CSRF: {fresh_csrf[:50] if fresh_csrf else '❌ GAGAL'}")

        try:
            t0 = _t.time()
            r = scraper.post(
                f"{BASE_URL}/portal/sms/received/getsms",
                data={"from": today, "to": today, "_token": fresh_csrf or sess.get("recv_csrf", "")},
                headers={
                    "Accept":           "text/html, */*; q=0.01",
                    "Content-Type":     "application/x-www-form-urlencoded; charset=UTF-8",
                    "X-Requested-With": "XMLHttpRequest",
            ...
            has_data = len(body) > 200 and "toggleRange" in body
            out.append(f"  POST getsms → HTTP {r.status_code} | size {len(body)}c | {elapsed:.2f}s")
            out.append(f"  URL final  : {r.url}")
            out.append(f"  Login page?: {'❌ YA (session expired!)' if is_login else '✅ Tidak'}")
            out.append(f"  Ada data?  : {'✅ YA (toggleRange ditemukan)' if has_data else '❌ Tidak ada range'}")
            out.append(f"  Body[0:200]: {body[:200]}")
        except Exception as e:
            out.append(f"  ❌ Error POST: {e}")

        out.append("")

    out.append(SEP)
    out.append("END DEBUG CSRF")
    out.append(SEP)
    out.append("")
    out.append("Hint endpoints lain:")
    out.append("  /debug/csrf?nocache=1           → bypass cache, selalu fresh scrape")
    out.append("  /debug/csrf?account=EMAIL       → fokus 1 akun")
    out.append("  /debug/full?date=DD/MM/YYYY     → full 3-step SMS debug")
    out.append("  /debug/dl-raw                   → debug export/download")
    ...
    def _try_download():
        ...
        for did in ["1", "2", "3"]:
            ...
            try:
                ...
                # Accept kalau: HTTP 200, ukuran > 1000, bukan halaman login/html biasa
                if fr.status_code == 200 and sz > 1000 and "/login" not in fr.url:
                    # Pastikan bukan HTML error page
                    ct_lower = ct.lower()
                    is_html_page = "text/html" in ct_lower and sz < 50000
                    if not is_html_page:
                        return fr, url
                    # Kalau HTML tapi gede (>50KB) mungkin memang file Excel yg salah CT
                    if sz > 50000:
                        logger.info(f"[EXPORT_DL] /download/{did} HTML tapi size {sz} > 50KB, diterima")
                        return fr, url
