...
def _scrape_jwt_token(scraper, page_url: str) -> str | None:
    """
    Scrape JWT token dari halaman iVAS portal.
    iVAS menyimpan token di:
      1. console.log("eyJ...") — di-print langsung ke JS console
      2. Variable JS: const driver = window.driver.js.driver
         (sebelum console.log biasanya ada token di-assign)
      3. URL WebSocket query param: ?token=eyJ...

    Return: JWT string atau None
    """
    try:
        r = scraper.get(page_url, timeout=20, allow_redirects=True)
        if "/login" in r.url:
            logger.warning(f"[WS-JWT] Session expired scraping {page_url}")
            return None

        html = decode_response(r)

        # ── Pattern 1: console.log("eyJxxx...") — CONFIRMED dari source iVAS ──
        m = re.search(
            r'console\.log\s*\(\s*["\']([A-Za-z0-9_\-]+\.[A-Za-z0-9_\-]+\.[A-Za-z0-9_\-]+)["\']',
            html
        )
        if m:
            tok = m.group(1)
            logger.info(f"[WS-JWT] ✅ JWT dari console.log: {tok[:30]}...")
            return tok

        # ── Pattern 2: ?token=eyJxxx dalam URL WS di JS ──
        m2 = re.search(
        ...
        if m2:
            tok = m2.group(1)
            logger.info(f"[WS-JWT] ✅ JWT dari URL token param: {tok[:30]}...")
            return tok

        # ── Pattern 3: token: 'eyJxxx' atau token: "eyJxxx" dalam JS ──
        m3 = re.search(
        ...
        if m3:
            tok = m3.group(1)
            logger.info(f"[WS-JWT] ✅ JWT dari token key: {tok[:30]}...")
            return tok

        # ── Pattern 4: Semua JWT-looking string (header.payload.signature) ──
        all_jwts = re.findall(
        ...
        if all_jwts:
            tok = all_jwts[0]
            logger.info(f"[WS-JWT] ✅ JWT pattern generic: {tok[:30]}...")
            return tok

        logger.warning(f"[WS-JWT] ❌ JWT tidak ditemukan di {page_url}")
    except Exception as e:
...
def _ws_connect_account(account: dict):
    ...
    while _ws_enabled:
        try:
            ...
            # ── Step 2: Scrape JWT token dari halaman portal ──
            jwt_token = _scrape_jwt_token(scraper, LIVE_PAGE)
            if not jwt_token:
                # Fallback: coba dari halaman lain
            ...
            if not jwt_token:
                logger.error(f"[WS] {email} — JWT tidak ditemukan, retry {reconnect_delay}s")
                _ws_set_status(email, error="JWT tidak ditemukan")
                fail_count += 1
                time.sleep(min(reconnect_delay * fail_count, 120))
                continue
            ...
            sio = _build_ws_client(email, jwt_token)
            if not sio:
                time.sleep(reconnect_delay)
                continue
            ...
            # ── Step 5: Connect dengan cookies session + JWT token ──
            # Ambil cookies dari scraper session
            ...
            logger.info(f"[WS] {email} → Connecting ke {WS_URL} ...")
            ...
            # ── Step 6: Kirim event online presence ──
            try:
...
def debug_ws():
    """
    Debug WebSocket — status detail + test JWT scraping per akun.
    ...
    for email, st in statuses.items():
        ...
        out.append(f"  JWT (prefix)   : {st.get('jwt_token','(belum)')}")
        ...
        if samples:
            out.append(f"  Sample SMS terbaru:")
            for s in samples:
    ...
    # Test JWT scraping live
    out.append(SEP2)
    out.append("JWT SCRAPE TEST (live, akun pertama)")
    accounts = _get_all_accounts() if '_get_all_accounts' in dir() else ACCOUNTS
    if accounts:
        acc  = accounts[0]
        sess = get_session(acc)
        if sess and sess.get("ok"):
            ...
            import time as _t
            t0  = _t.time()
            tok = _scrape_jwt_token(scraper, f"{BASE_URL}/portal/live/test_sms")
            elapsed = _t.time() - t0
            out.append(f"  Akun    : {acc['email']}")
            out.append(f"  Token   : {tok[:50] + '...' if tok else '❌ TIDAK DITEMUKAN'}")
            out.append(f"  Elapsed : {elapsed:.2f}s")
        else:
            out.append("  ❌ Login gagal untuk akun pertama")
    out.append("")
    out.append(SEP)
